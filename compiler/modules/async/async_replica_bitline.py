...
from async_replica_bitcell import replica_bitcell
...
class replica_bitline(design.design):
    ...
    def calculate_module_offsets(self):
        ...
        self.replica_bitcell_offset = vector(self.rbl_inv_offset.x+2*self.m2_pitch, 
        ...
        self.height = max(self.replica_bitcell_offset.y + (self.bitcell_loads+1)*self.replica_bitcell.height, 
                          self.delay_chain_offset[0].y+self.delay_chain[0].width+2*self.m1_pitch)
        
        self.width = self.replica_bitcell_offset.x + self.replica_bitcell.width + 3*self.m1_pitch+(self.stage-1)*self.delay_chain[0].height
    ...
    def add_rbl_well_contacts(self):
        ...
        vdd_start = self.replica_bitcell_offset.x + self.replica_bitcell.width+\
                    self.m1_pitch+0.5*contact.m1m2.width
    ...
    def add_well_contact(self, contact_offset, well_offset, implant_offset, 
            ...
            self.add_contact(("active", "contact", "m1"), 
    ...
    def route_access_tx(self):
        ...
        self.add_path("m1", [delay_en_offset,contact_offset])
        ...
        self.add_path("m1", [contact_offset, wl_mid, (wl_offset.x+self.m1_width,wl_offset.y)])
        ...
        self.add_path("m1", [drain_offset, vdd_offset])
        ...
        self.add_path("m1", [source_offset, mid1, mid2, inv_A_offset])
        ...
        self.add_wire(self.m1_stack, [pos1, pos2, pos5], widen_short_wires=False)
        
    def route_vdd(self):
        """ Adds two vdd pins, one for replica bitline load and one for delay chain """        
        
        # Add first vdd pin to the right of the rbc 
        vdd_start = vector(self.replica_bitcell_offset.x + self.replica_bitcell.width+self.m1_pitch ,0)
        ...
        for rbc_vdd in self.rbc_inst.get_pins("vdd"):
            ...
            if rbc_vdd.layer == "m3pin":
                ...
                self.add_wire(self.m1_stack, [rbc_vdd.bc(), 
                                             (rbc_vdd.bc().x, rbc_vdd.bc().y-2*self.m1_pitch),
                                             (vdd_start.x, rbc_vdd.bc().y-2*self.m1_pitch)], widen_short_wires=False)
        ...
        for i in range(self.stage-1):
            ...
            pos2=vector(pos1.x, self.dc_inst[i].uy() + self.m1_pitch)
            pos4=self.dc_inst[i+1].get_pin("vdd").uc()
            pos3=vector(pos4.x, self.dc_inst[i].uy() + self.m1_pitch)
            self.add_wire(self.m1_rev_stack, [pos1, pos2, pos3, pos4], widen_short_wires=False)
            
            pos1=self.dc_inst[i].get_pin("gnd").uc()
            pos2=vector(pos1.x, self.dc_inst[i].uy() + 2*self.m1_pitch)
            pos4=self.dc_inst[i+1].get_pin("gnd").uc()
            pos3=vector(pos4.x, self.dc_inst[i].uy() + 2*self.m1_pitch)
            self.add_path("m1", [pos1, pos2, pos3, pos4], width=contact.m1m2.width)
    ...
    def route_gnd(self):
        ...
        #yoff=self.rbc_inst.get_pin("wl").lc().y+self.m1_pitch
        ...
        for rbc_gnd in self.rbc_inst.get_pins("gnd"):
            ...
            if rbc_gnd.layer == "m3pin":
                ...
                self.add_wire(self.m1_stack, [rbc_gnd.bc(), 
                                             (rbc_gnd.bc().x, rbc_gnd.bc().y-3*self.m1_pitch),
                                             (gnd_pin.x, rbc_gnd.bc().y-3*self.m1_pitch)], widen_short_wires=False)
    ...
    def add_layout_pins(self):
        ...
        shift = contact.m1m2.height + self.minarea_m1/contact.m1m2.height
        for i in range(self.stage-1):         
            pos1=self.dc_inst[i].get_pin("in").uc()
            pos4=self.dc_inst[i+1].get_pin("out").uc()
            if i%2:
                pos2=vector(pos1.x, pos1.y-max(2*shift, 2*self.m1_pitch))
                pos3=vector(pos4.x, pos1.y-max(2*shift, 2*self.m1_pitch))
                self.add_wire(self.m1_rev_stack, [pos1, pos2, pos3, pos4], widen_short_wires=False)
            else:
        ...
        x_off1 = self.dc_inst[self.stage-1].lx()-self.m1_pitch
        off2 = self.dc_inst[self.stage-1].get_pin("in").uc()
        self.add_wire(self.m1_rev_stack, 
                      [(x_off1, 0), (x_off1, self.dc_inst[self.stage-1].by()-3*self.m1_pitch),
                       (off2.x, self.dc_inst[self.stage-1].by()-3*self.m1_pitch), off2], widen_short_wires=False)
