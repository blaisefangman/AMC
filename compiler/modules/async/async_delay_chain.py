...
class delay_chain(design.design):
    ...
    def __init__(self, num_inv, num_stage, name="delay_chain"):
        ...
        self.create_module()
        self.route_inv()
        self.width = (self.num_even_stage+self.final_stage)*self.inv.width + 4*self.m1_pitch+contact.m1m2.height
    ...
    def route_inv(self):
        ...
        for j in range(self.final_stage_size):
            ...
            mid_pos1=vector(pin1.rx()+self.m1_pitch, pin1.lc().y)
            mid_pos2=vector(mid_pos1.x, (j+1)*self.inv.height)
            mid_pos3=vector(pin2.lx()-self.m1_pitch, mid_pos2.y)
            mid_pos4=vector(mid_pos3.x, pin2.lc().y)
            self.add_path("m2", [mid_pos1, mid_pos2, mid_pos3, mid_pos4])
            self.add_via_center(self.m1_stack, mid_pos1, rotate=90)
            self.add_via_center(self.m1_stack, mid_pos4, rotate=90)
            self.add_path("m1", [pin1.lc(), mid_pos1])
            self.add_path("m1", [ mid_pos4, pin2.lc()])
        ...
        for i in range(self.stage_size):
            for j in range(self.num_even_stage-1):
                ...
                self.add_rect(layer="m1", offset= off, width=self.m1_space, height=height)
    ...
    def add_layout_pins(self):
        ...
        for i in range(2):
            ...
            self.add_rect(layer="m2", 
                          offset = (xoff-(2+i)*self.m1_pitch,0),
                          width=self.m2_width,
                          height=self.height)
        for i in range(self.stage_size):
            if i%2:
                pin = self.inv_inst[i].get_pin("gnd")
                self.add_path("m1", [(xoff-2*self.m1_pitch, pin.lc().y), pin.lc()])
                self.add_via_center(self.m1_stack, (xoff-2*self.m1_pitch+0.5*self.m2_width, pin.lc().y), rotate=90)

            else:
                pin = self.inv_inst[i].get_pin("vdd")
                self.add_path("m1", [(xoff-3*self.m1_pitch, pin.lc().y), pin.lc()])
                self.add_via_center(self.m1_stack, (xoff-3*self.m1_pitch+0.5*self.m2_width, pin.lc().y), rotate=90)
        ...
        for i in range(2):
            ...
            self.add_path("m1", [(xoff-(2+i)*self.m1_pitch, pin.lc().y), pin.lc()])
            self.add_via_center(self.m1_stack, (xoff-(2+i)*self.m1_pitch+0.5*self.m2_width, pin.lc().y), rotate=90)
