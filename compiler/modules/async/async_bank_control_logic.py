...
from async_nor3 import nor3
from async_nor2 import nor2
from async_bitcell import bitcell
from async_delay_chain import delay_chain
from async_pull_up_pull_down import pull_up_pull_down 
...
class bank_control_logic(design.design):
    ...
    def create_layout(self):
        ...
        if self.num_subanks > 1:
            ...
            self.pin_width = self.dr_inst.rx() + 4*self.m1_pitch
        else:
            if self.two_level_bank:
                # rreq_merge_inv_inst + 3*m1_pitch for connections
                self.pin_width = self.rreq_mrg_inv1_inst.rx() + 3*self.m1_pitch
            else:
                # decoder_enable_inst + 1*m1_pitch for connections
                self.pin_width = self.dec_en_inst.rx() + self.m1_pitch
        
        if self.power_gate:
            self.pin_width = self.pin_width+3*self.inv.height+4*self.m1_pitch+2*self.gap

        self.dc = delay_chain(num_inv=self.num_inv, num_stage=self.num_stage)
        self.add_mod(self.dc)
        self.add_dc(0)

        self.height= self.dc_inst.uy() - self.min_off_y + 3*self.m1_pitch

        #delay chain inst + + 5*m1_pitch for connections
        self.pin_width = max(self.dc.height+5*self.m1_pitch, self.pin_width) 
        
        if self.power_gate:
            self.add_power_gate_cells()
        
        #total width= pin_width + 3*m1_pitch for spaces for both sides
        self.width = self.pin_width + 3*self.m1_pitch
    ...
    def setup_layout_offsets(self):
        ...
        self.gap= max(self.implant_space, self.well_space, self.m1_pitch)
        ...
        self.wen_off_y = v_shift+2*self.m2_pitch
        self.pchg_off_y = self.wen_off_y+self.m2_pitch
        self.reset_bar_off_y = self.pchg_off_y+self.m2_pitch
        if self.num_subanks > 1:
            self.comp_off_y = self.reset_bar_off_y            
        self.xsen_off_y = self.U_off_y = self.reset_bar_off_y+self.m2_pitch
        self.sen_off_y = self.U_off_y+self.m2_pitch
        self.vdd_off_y = self.sen_off_y+self.m2_pitch
        self.gnd_off_y = self.vdd_off_y+self.m2_pitch
        
        # Y-offset of input/output pins blow the gates
        self.r_off_y = -2*self.m2_pitch
        self.wc_off_y = self.r_off_y-self.m2_pitch
        self.rw_off_y = self.wc_off_y-self.m2_pitch
        self.dr_off_y = self.rw_off_y-self.m2_pitch
        self.ack_off_y = self.dr_off_y-self.m2_pitch
        self.wack_off_y = self.ack_off_y-self.m2_pitch
        self.rreq_off_y = self.wack_off_y-self.m2_pitch
        self.data_ready_off_y = self.rreq_off_y-self.m2_pitch
        self.w_off_y = self.data_ready_off_y-self.num_subanks*self.m2_pitch
        self.write_comp_off_y = self.w_off_y-self.m2_pitch
        self.rack_off_y = self.write_comp_off_y-self.num_subanks*self.m2_pitch
        self.reset_off_y = self.rack_off_y
        self.go_off_y = self.reset_off_y-self.m2_pitch
        self.dec_en_off_y =self.go_off_y-(self.num_subanks+1)*self.m2_pitch
        self.min_off_y = self.dec_en_off_y - 2*self.m2_pitch
        if self.power_gate:
            self.sleep_off_y = self.min_off_y - self.m2_pitch
            self.min_off_y = self.sleep_off_y - 2*self.m2_pitch            
    ...
    def add_modules(self):
        ...
        if self.two_level_bank:
            ...
            self.rreq_mrg_inv1_inst_lr_x=self.wack_inst.rx()+2*self.inv.height+2*self.m1_pitch
        if self.num_subanks > 1: 
            self.add_WC_OR_gate()
            self.add_DR_OR_gate()
    ...
    def add_Zpath(self, pin1, pin2, gap):
        ...
        self.add_path("m1",[pin1.uc(), (pin1.uc().x, pin1.lc().y-gap), 
    ...
    def add_Zwire(self, pin1, pin2, gap):
        ...
        self.add_wire(self.m1_rev_stack,[pin1.uc(), (pin1.uc().x, pin1.lc().y-gap), 
                                        (pin2.uc().x, pin1.lc().y-gap), pin2.uc()], widen_short_wires=False)
    ...
    def add_pchg_gate(self):
        ...
        self.add_path("m1",[self.pchg_inst.get_pin("Z").uc(), 
    ...
    def add_u_gate(self):
        ...
        u_off= (self.pchg_inv_inst.rx() + self.u_gate.height + 2*self.m1_pitch, 0)
        ...
        self.add_Zpath(self.u_inst.get_pin("Dn0"), self.u_inst.get_pin("Dp1"), self.m1_pitch)
        ...
        self.add_m1P_minarea(vector(self.u_inst.get_pin("Gp0").lc().x+self.co_xshift,self.u_inst.get_pin("Gp0").lc().y), 
                             self.u_inst.by(),0)
        self.add_m1P_minarea(self.u_inst.get_pin("Gn0").lr(), self.u_inst.uy()+self.m1_pitch, 0) 

    def add_wen_gate(self):
        """ Adds the pull_up_pull_down network for wen-gate """
        
        #add 3*m1_pitch for 3 connections of wen_gate
        wen_off= (self.u_inst.rx()+3*self.m1_pitch,0)
        ...
        #Adding a Z connection for output node
        self.add_Zpath(self.wen_inst.get_pin("Dn0"),self.wen_inst.get_pin("Dp1"),self.m1_pitch) 

        #Connecting the output node to inv input
        self.add_Zpath(self.wen_inv_inst.get_pin("A"),self.wen_inst.get_pin("Dn0"),self.m1_pitch)
        #self.add_path("m1",[self.wen_inst.get_pin("Dn0").uc(),self.wen_inv_inst.get_pin("A").lc()])
        
        #Adding a Z connection for active contact connections
        self.add_Zwire(self.wen_inst.get_pin("Sn0"), self.wen_inst.get_pin("Dn2"),self.m1_pitch)
        
        #Connecting the vdd of inv to wen_inst, gnd is connected by abutment
        self.add_path("m1", [self.wen_inst.get_pin("vdd").uc(), 
    ...
    def add_sen_gate(self):
        ...
        #Adding a Z connection for output node
        self.add_Zpath(self.sen_inst.get_pin("Dp1"),self.sen_inst.get_pin("Dn0"),-self.m1_pitch)
        
        #Connecting the output node to inv input
        self.add_Zpath(self.sen_inv_inst.get_pin("A"),self.sen_inst.get_pin("Dn0"),self.m1_pitch)
        #self.add_path("m1",[self.sen_inst.get_pin("Dn0").uc(),self.sen_inv_inst.get_pin("A").lc()])

        #Adding a Z connection for active contact connections
        pos1=self.sen_inst.get_pin("Sn0").uc()
        pos2=self.sen_inst.get_pin("Dn1").uc()
        gap=3*self.m1_pitch
        self.add_path("m1", [pos1, (pos1.x, pos1.y+gap)])
        self.add_path("m1", [pos2, (pos2.x, pos2.y+gap)])
        self.add_path("m2", [(pos1.x, pos1.y+gap), (pos2.x, pos2.y+gap)], width=contact.m1m2.height)
        ...
        self.add_path("m1", [self.sen_inst.get_pin("vdd").uc(),
    ...
    def add_ack_gate(self):
        ...
        ack_off= (self.sen_inst.rx()+6*self.m1_pitch,0)
        ...
        #Adding a Z connection for active contact connections
        self.add_Zwire(self.ack_inst.get_pin("Dn0"),self.ack_inst.get_pin("Dn4"),-self.m2_pitch)
        self.add_Zpath(self.ack_inst.get_pin("Dn3"),self.ack_inst.get_pin("Dn5"),self.m1_pitch) 

        #Adding a Z connection for output node
        self.add_Zpath(self.ack_inst.get_pin("Sn0"), self.ack_inst.get_pin("Dp2"),self.m1_pitch) 
        
        #Connecting output of ack_inst to inv input
        self.add_path("m1",[self.ack_inst.get_pin("Sn0").uc(),
                               (self.ack_inv_inst1.get_pin("A").lr()-vector(0,0.5*self.m1_width))])
        self.add_path("m1",[self.ack_inv_inst1.get_pin("Z").uc(),
                               self.ack_inv_inst2.get_pin("A").lc()])
        self.add_path("m1",[self.ack_inv_inst2.get_pin("Z").uc(),
                                self.ack_inv_inst.get_pin("A").lc()])
        
        #Connecting the vdd of inv to ack_inst vdd, gnd is connected by abutment
        self.add_path("m1", [self.ack_inst.get_pin("vdd").uc(), 
        ...
        self.add_m1P_minarea(vector(self.ack_inst.get_pin("Gn0").rx()-contact.poly.width, self.ack_inst.get_pin("Gn0").by()), 
                             self.ack_inst.by(),-self.m1_pitch, self.m1_width)
        self.add_m1P_minarea(vector(self.ack_inst.get_pin("Gn3").rx()+self.co_xshift-contact.poly.width, self.ack_inst.get_pin("Gn3").lc().y),
                             self.ack_inst.by(),2*self.m1_pitch)
    ...
    def add_rack_gate(self):
        ...
        rack_off= (self.ack_inst.rx()+self.rack_gate.height+4*self.m1_pitch,0)
        ...
        #Adding a Z connection for output node
        self.add_Zpath(self.rack_inst.get_pin("Dn2"),self.rack_inst.get_pin("Dp1"),self.m1_pitch+self.m1_space)

        #Connecting output of rack_inst to inv input
        self.add_Zpath(self.rack_inst.get_pin("Dn2"),self.rack_inv_inst1.get_pin("A"),-self.m1_pitch-self.m1_width)
        self.add_path("m1", [self.rack_inv_inst1.get_pin("Z").uc(),self.rack_inv_inst2.get_pin("A").lc()])
        self.add_path("m1", [self.rack_inv_inst2.get_pin("Z").uc(),self.rack_inv_inst.get_pin("A").lc()])
        
        #Connecting the vdd and gnd of inv to rack_inst
        self.add_path("m1", [self.rack_inst.get_pin("vdd").uc(),
                                self.rack_inv_inst.get_pin("vdd").lc()], width=contact.m1m2.width)
        self.add_path("m1", [self.rack_inst.get_pin("gnd").uc(),
    ...
    def add_wack_gate(self):
        ...
        wack_off= (self.rack_inst.rx()+5*self.m1_pitch,0)
        ...
        #Adding a Z connection for output node
        self.add_Zpath(self.wack_inst.get_pin("Dn2"),self.wack_inst.get_pin("Dp1"),self.m1_pitch)
        ...
        self.add_path("m1", [self.wack_inv_inst1.get_pin("A").uc(),
                                 pos1, pos2, pos3, self.wack_inst.get_pin("Dn2").ul()])
        self.add_path("m1", [self.wack_inv_inst1.get_pin("Z").uc(),self.wack_inv_inst2.get_pin("A").lc()])
        self.add_path("m1", [self.wack_inv_inst2.get_pin("Z").uc(),self.wack_inv_inst.get_pin("A").lc()])
        
        #Connecting the vdd and gnd of inv to wack_inst
        self.add_path("m1", [self.wack_inst.get_pin("vdd").uc(), 
                                 self.wack_inv_inst.get_pin("vdd").lc()], width=contact.m1m2.width)
        self.add_path("m1", [self.wack_inst.get_pin("gnd").uc(), 
    ...
    def add_dec_en_gate(self):
        ...
        delay_inv_off= (self.wack_inst.rx()+4*self.m1_pitch+\
    ...
    def add_rreq_merge_enable(self):
        ...
        rreq_mrg_inv_off= (self.wack_inst.rx()+ 2* self.inv.height+4*self.m1_pitch+\
                           0.5*contact.m1m2.width,self.inv5.width)        
        ...
        self.rreq_mrg_inv2_inst=self.add_inst(name="rreq_merge_inv_1", 
                                              mod=self.inv, 
                                              offset=rreq_mrg_inv_off+vector(0,self.inv.width+3*self.m1_pitch), 
                                              mirror="MX", 
                                              rotate=270)
        ...
        mid_pos1=(rreq_mrg_in_off1.x, rreq_mrg_in_off1.y+self.m1_pitch)
        mid_pos2=(rreq_mrg_out_off1.x, rreq_mrg_in_off1.y+self.m1_pitch)
        
        #Connecting output of rreq_inv2 to rreq_inv2 input
        self.add_path("m1", [rreq_mrg_in_off1, mid_pos1, mid_pos2, rreq_mrg_out_off1])


        rreq_mrg_in_off=self.rreq_mrg_inv2_inst.get_pin("A").uc()
        rreq_mrg_out_off=self.rreq_mrg_inv1_inst.get_pin("Z").uc()
        self.add_path("m1", [rreq_mrg_out_off,(rreq_mrg_out_off.x, self.min_off_y)])
        ...
        pos1=rreq_mrg_in_off+vector(0,self.m1_pitch)
        pos2=(self.rreq_mrg_inv2_inst.rx()+self.m1_pitch,pos1[1])
        pos3=(pos2[0], self.rreq_off_y)
        self.add_wire(self.m1_rev_stack, [rreq_mrg_in_off,pos1, pos2, pos3], widen_short_wires=False)
        
        self.add_via(self.m1_stack,(pos2[0],self.rreq_off_y), rotate=90)
                           
        # Calculating the offset for next cell
        self.rreq_mrg_inv1_inst_lr_x = self.rreq_mrg_inv1_inst.rx()+2*self.m1_pitch
    ...
    def add_WC_OR_gate(self):
        ...
        wc_off= (self.rreq_mrg_inv1_inst_lr_x+self.wc_gate.height+2*self.m1_pitch,
                 self.wc_gate.width)
        ...
        for i in range(self.num_subanks//2 - 1):
            self.add_Zpath(Dn_contact[i], Dn_contact[i+1], -1.5*self.m1_pitch)

        #Adding a Z connection for output node
        Dp_contact = self.wc_inst.get_pin("Dp0")
        self.add_path("m1", [Dn_contact[self.num_subanks//2-1].uc(), Dp_contact.uc()])
        
        #Connecting output of wc_inst to inv input
        self.add_Zpath(self.wc_inv_inst.get_pin("A"),Dp_contact,self.m1_pitch)
        
        #Connecting the vdd of inv to wc_inst, gnd is connected by abutment
        self.add_path("m1", [self.wc_inst.get_pin("gnd").uc(), 
                                 self.wc_inv_inst.get_pin("gnd").lc()], width=contact.m1m2.width)
        self.add_path("m1", [self.wc_inst.get_pin("vdd").uc(), 
        ...
        self.add_m1P_minarea(self.wc_inst.get_pin("Gp0").lr(),self.wc_inst.uy(),self.m1_pitch)
    ...
    def add_DR_OR_gate(self):
        ...
        self.add_Zpath(self.dr_inv_inst.get_pin("A"), Dp_contact, self.m1_pitch)
        
        #Connecting the vdd of inv to dr_inst, gnd is connected by abutment
        self.add_path("m1", [self.dr_inst.get_pin("gnd").uc(), 
                                 self.dr_inv_inst.get_pin("gnd").lc()], width=contact.m1m2.width)
        self.add_path("m1", [self.dr_inst.get_pin("vdd").uc(), 
        ...
        self.add_m1P_minarea(self.dr_inst.get_pin("Gp0").lr(),self.dr_inst.uy(),self.m1_pitch)
        for i in range(2*self.num_subanks-1):
    ...
    def add_dc(self,y_off):
        ...
        dc_off= (self.dc.height, self.gnd_off_y+3*self.m1_pitch)
    ...
    def add_power_gate_cells(self):
        ...
        if self.num_subanks > 1:
            xoff = self.dr_inst.rx() + 2*self.m1_pitch
        ...
        for i in range(3):
            ...
            self.add_path("m1", [(pin1.uc().x, pin1.uy()-self.m1_width), (pin2.uc().x, pin2.by()+self.m1_width)])
    ...
    def add_input_rw_pin(self):
        ...
        self.add_via(self.m1_stack,(pchg_B.x, self.rw_off_y - self.v1_via_shift))
        
        self.add_path("m1",[self.pchg_inst.get_pin("B").uc(), 
        ...
        self.add_contact(self.poly_stack,(self.wen_rw_in_off.rx()-contact.poly.width, 0-self.co_via_shift))
        self.add_path("m1",[(self.wen_rw_in_off.rx()-contact.poly.width+0.5*contact.poly.width, 0),
                                (self.wen_rw_in_off.rx()-contact.poly.width+0.5*contact.poly.width, self.rw_off_y)])
        self.add_via(self.m1_stack,(self.wen_rw_in_off.rx()-contact.poly.width, self.rw_off_y - self.v1_via_shift))
        
        # sen-gate rw-input connection
        self.add_path("poly",[self.sen_rw_in_off.uc(), (self.sen_rw_in_off.uc().x, 0)])
        self.add_contact(self.poly_stack, (self.sen_rw_in_off.lx(),0-self.co_via_shift))
        self.add_path("m1",[(self.sen_rw_in_off.lx()+0.5*contact.poly.width,0), 
                                (self.sen_rw_in_off.lx()+0.5*contact.poly.width, self.rw_off_y)])
        self.add_via(self.m1_stack, (self.sen_rw_in_off.lx(), self.rw_off_y - self.v1_via_shift))
        ...
        pos4=(pos3[0], -self.m1_pitch)
        self.add_path("poly", [pos1, pos2, pos3, pos4])
        self.add_contact(self.poly_stack,(self.ack_rw_in_off.lx()+self.poly_to_active, -self.m1_pitch-self.co_via_shift))
        self.add_path("m1",[(self.ack_rw_in_off.lx()+0.5*contact.poly.width+self.poly_to_active,-self.m1_pitch), 
                                (self.ack_rw_in_off.lx()+0.5*contact.poly.width+self.poly_to_active,self.rw_off_y)])
        self.add_via(self.m1_stack, (self.ack_rw_in_off.lx()+self.poly_to_active, self.rw_off_y - self.v1_via_shift))
    ...
    def add_input_w_pin(self):
        ...
        self.add_via(self.m1_stack, (pchg_C.x, self.w_off_y - self.v1_via_shift))
        self.add_path("m1",[self.pchg_inst.get_pin("C").uc(), 
        ...
        self.add_contact(self.poly_stack, (self.wen_w_in_off.rx()-contact.poly.width, 2*contact.poly.height-self.co_via_shift))
        self.add_path("m1",[(self.wen_w_in_off.rx()-contact.poly.width+0.5*contact.poly.width, 2*contact.poly.height), 
                                (self.wen_w_in_off.rx()-contact.poly.width+0.5*contact.poly.width, self.w_off_y)])
        self.add_via(self.m1_stack, (self.wen_w_in_off.rx()-contact.poly.width, self.w_off_y - self.v1_via_shift))
        ...
        pos4=(pos3[0], -3*self.m1_pitch)
        self.add_path("poly", [pos1, pos2, pos3, pos4])
        self.add_contact(self.poly_stack, (self.ack_w_in_off.rx()+self.poly_to_active-contact.poly.width, -3*self.m1_pitch-self.co_via_shift))
        
        pos1=(self.ack_w_in_off.rx()+self.poly_to_active+0.5*contact.poly.width-contact.poly.width,-3*self.m1_pitch)
        pos2=(pos1[0],self.w_off_y)
        self.add_path("m1",[pos1, pos2])
        self.add_via(self.m1_stack, (self.ack_w_in_off.rx()+self.poly_to_active-contact.poly.width, self.w_off_y - self.v1_via_shift))
    ...
    def add_input_r_pin(self):
        ...
        self.add_rect(layer="m2",
                      ...
                      width=self.ack_inst.lx()-3*self.m1_pitch-pchg_A.x,
                      height = contact.m1m2.width)
        self.add_via(self.m1_stack, (pchg_A.x, self.r_off_y - self.v1_via_shift))
        self.add_path("m1",[self.pchg_inst.get_pin("A").uc(),
        ...
        self.add_contact(self.poly_stack, (self.sen_r_in_off.rx(), 2*contact.poly.height-self.co_via_shift))

        self.add_path("m1",[(self.sen_r_in_off.rx()+0.5*contact.poly.width, 2*contact.poly.height), 
        ...
        self.add_contact(self.poly_stack,(self.ack_r_in_off.lx()-self.via_co_shift, 
                                          self.ack_inst.uy()-contact.poly.height+self.co_via_shift))
        ...
        pos2=(self.ack_inst.lx()-3*self.m1_pitch, pos1[1])
        pos3=(pos2[0], self.r_off_y)
        self.add_wire(self.m1_rev_stack, [pos1, pos2, pos3], widen_short_wires=False)
        self.add_via(self.m1_stack, (pos2[0]+0.5*self.m1_width+ self.v1_via_shift,self.r_off_y), rotate=90)
    ...
    def add_sleep_pin(self):
        ...
        self.add_path("m1", [(self.sleep_in_off.uc().x, self.pg_inv[0].by()), (self.sleep_in_off.uc().x, self.min_off_y)], width=contact.m1m2.width)
        
        pos1=(self.pg_nor[0].get_pin("B").lx(),self.sleep_off_y-self.m2_pitch)
        pos2=(self.pg_nor[2].get_pin("B").lx(),self.sleep_off_y-self.m2_pitch)
        ...
        for i in range(3):
            ...
            pos2=(pos.uc().x, self.sleep_off_y-self.m2_pitch)
            self.add_path("m1",[pos1 , pos2], width=contact.m1m2.width)
            self.add_path("m2",[(pos.uc().x, pos.by()) , (pos.uc().x, self.pg_inv[0].by())])
    ...
    def add_input_reset_pin(self):
        ...
        self.add_path("m1",[self.rst_in_off.uc(), (self.rst_in_off.uc().x,self.min_off_y)])
        
        # U-gate reset input 
        self.add_rect(layer="m2", 
                      offset=(self.rst_in_off.uc().x, self.reset_off_y), 
                      width=self.u_inst.rx()+self.m1_pitch-self.rst_in_off.uc().x,
                      height = contact.m1m2.width)
        self.add_via(self.m1_stack, (self.rst_in_off.uc().x,self.reset_off_y - self.v1_via_shift))
        self.add_path("poly",[self.u_rst_in_off.uc(), 
                             (self.u_rst_in_off.uc().x,self.u_gate.width+contact.poly.height+self.m1_pitch)])
        self.add_contact(self.poly_stack, 
                         (self.u_rst_in_off.rx()-self.via_co_shift, self.u_gate.width+self.m1_pitch+self.co_via_shift))
        self.add_via(self.m1_stack, (self.u_rst_in_off.rx(), self.u_gate.width+self.m1_pitch))
        
        pos1=(self.u_rst_in_off.rx(), self.u_gate.width+self.m1_pitch+0.5*self.m2_width)
        pos2=(self.u_inst.rx()+self.m1_pitch, pos1[1])
        pos3=(pos2[0], self.reset_off_y)
        self.add_wire(self.m1_rev_stack, [pos1, pos2, pos3], widen_short_wires=False)
        self.add_via(self.m1_stack, (pos2[0], self.reset_off_y - self.v1_via_shift))

        # reset-gate reset_bar connection
        self.add_rect(layer="m2", 
                      offset=(self.rst_out_off.uc().x,self.reset_bar_off_y), 
                      width=self.wack_inst.rx()+self.m1_pitch-self.rst_out_off.uc().x,
                      height = contact.m1m2.width)

        self.add_via(self.m1_stack, (self.rst_out_off.lx(),self.reset_bar_off_y - self.v1_via_shift))
        self.add_path("m1",[self.rst_out_off.uc(),
                               (self.rst_out_off.uc().x,self.reset_bar_off_y)])

        # ack-gate reset_bar connection
        self.add_path("poly",[self.ack_reset_bar_in_off.uc(), (self.ack_reset_bar_in_off.uc().x, -2*self.m1_pitch)])
        self.add_contact(self.poly_stack, (self.ack_reset_bar_in_off.rx()-self.via_co_shift+self.co_xshift-contact.poly.width, -2*self.m1_pitch-self.co_via_shift))
        self.add_via(self.m1_stack, (self.ack_reset_bar_in_off.rx()+self.co_xshift-contact.poly.width, -2*self.m1_pitch))

        pos1=(self.ack_reset_bar_in_off.uc().x,-2*self.m1_pitch+0.5*self.m2_width)
        pos2=(self.ack_inst.lx()-self.m1_pitch, pos1[1])
        pos3=(pos2[0], self.reset_bar_off_y)
        self.add_wire(self.m1_rev_stack, [pos1, pos2, pos3], widen_short_wires=False)
        self.add_via(self.m1_stack, (pos2[0]-0.5*self.m1_width, self.reset_bar_off_y - self.v1_via_shift))

        # rack-gate reset_bar connection
        self.add_path("poly",[self.rack_rst_b_in_off.uc(), 
                             (self.rack_rst_b_in_off.uc().x,-self.m1_pitch)])
        self.add_contact(self.poly_stack,(self.rack_rst_b_in_off.lx()-self.via_co_shift, -self.m1_pitch-self.co_via_shift))
        self.add_via(self.m1_stack, (self.rack_rst_b_in_off.lx(), -self.m1_pitch))
        
        pos1= (self.rack_rst_b_in_off.uc().x,-self.m1_pitch+0.5*self.m2_width)
        pos2= (self.rack_inst.lx()-2*self.m1_pitch,pos1[1])
        pos3= (pos2[0], self.reset_bar_off_y)
        self.add_wire(self.m1_rev_stack, [pos1, pos2, pos3], widen_short_wires=False)
        self.add_via(self.m1_stack, (pos2[0]-0.5*self.m1_width, self.reset_bar_off_y - self.v1_via_shift))
        ...
        self.add_contact(self.poly_stack,(self.wack_rst_b_in_off.lx()-self.via_co_shift+self.co_xshift, 2*contact.poly.height-self.co_via_shift))
        ...
        pos2= (self.wack_inst.rx()+self.m1_pitch,pos1[1])
        pos3= (pos2[0], self.reset_bar_off_y)
        self.add_wire(self.m1_rev_stack,[pos1, pos2, pos3], widen_short_wires=False)
        self.add_via(self.m1_stack, (pos2[0]-0.5*self.m1_width, self.reset_bar_off_y - self.v1_via_shift))
    ...
    def add_input_wreq_pin(self):
        ...
        self.add_contact(self.poly_stack,(self.wack_wreq_in_off.lx()+self.co_xshift, 0-self.co_via_shift))
        self.add_path("m1",[(self.wack_wreq_in_off.lx()+0.5*self.m1_width+2*self.co_xshift, contact.poly.height), 
    ...
    def add_input_rreq_pin(self):
        ...
        self.add_contact(self.poly_stack,(self.rack_rreq_in_off.lx()+self.co_xshift, 0-self.co_via_shift))
        self.add_path("m1",[(self.rack_rreq_in_off.lx()+2*self.co_xshift+0.5*self.m1_width, 0), 
        ...
        self.add_rect(layer="m2",
                      ...
                      width= self.pin_width-self.rack_rreq_in_off.uc().x-self.m1_pitch,
                      height=self.m2_width)
        self.add_via(self.m1_stack, (self.rack_rreq_in_off.uc().x,self.rreq_off_y - self.v1_via_shift))
    ...
    def add_input_dr_pins(self):
        ...
        if self.num_subanks > 1:
            for i in range(self.num_subanks):
                self.add_path("poly",[self.DR_off[i].uc(), 
                                     (self.DR_off[i].uc().x, -2*i*self.m1_pitch)])
                self.add_contact(self.poly_stack,
                                 (self.DR_off[i].lx(),-2*i*self.m1_pitch-self.co_via_shift))

                x_off=self.DR_off[i].lx()+0.5*contact.poly.width
                self.add_path("m1",[(x_off, -2*i*self.m1_pitch),
                                        (x_off, self.data_ready_off_y- i*self.m2_pitch)])
                if ((-self.data_ready_off_y- i*self.m2_pitch) < (self.minarea_m1/contact.m1m2.width)):
                    self.add_m1_minarea((self.DR_off[i].lx()+self.co_xshift+0.5*contact.m1m2.width, self.data_ready_off_y-(i-1)*self.m2_pitch))
                self.add_via(self.m1_stack, (self.DR_off[i].lx()+self.co_xshift,
                                             self.data_ready_off_y- i*self.m2_pitch-self.v1_via_shift))

                self.add_rect(layer="m2", 
                              offset=(-self.m1_pitch,
                                      self.data_ready_off_y- i*self.m2_pitch),
                              width=self.width,
                ...
                self.add_rect(layer="m2", 
                              offset=(-self.m1_pitch,self.dr_off_y), 
        ...
        self.add_path("poly",[self.rack_dr_in_off.uc(), 
                             (self.rack_dr_in_off.uc().x, self.rack_inst.uy()+self.m1_pitch)])
        self.add_contact(self.poly_stack,(self.rack_dr_in_off.lx()-self.via_co_shift, 
                                          self.rack_inst.uy()+self.m1_pitch-contact.poly.height+self.co_via_shift))
        self.add_via(self.m1_stack, (self.rack_dr_in_off.lx(), 
                                      self.rack_inst.uy()+self.m1_pitch-contact.m1m2.height))

        pos1= (self.rack_dr_in_off.uc().x, self.rack_inst.uy()+self.m1_pitch-0.5*self.m2_width)
        pos2= (self.rack_inst.lx()-self.m1_pitch,pos1[1])
        pos3= (pos2[0],self.dr_off_y)
        self.add_wire(self.m1_rev_stack,[pos1, pos2, pos3], widen_short_wires=False)
        self.add_via(self.m1_stack, (pos2[0], self.dr_off_y - self.v1_via_shift))
        
        # Add data_ready-gate output
        if self.num_subanks > 1:
            x_off = self.dr_inst.rx()+2*self.m1_pitch
            self.add_via(self.m1_stack, (x_off, self.dr_off_y - self.v1_via_shift))
            self.add_wire(self.m1_rev_stack,[self.dr_off.uc(),
                           ...
                           (x_off, self.dr_off.uc().y),(x_off, self.dr_off_y)], widen_short_wires=False)
    ...
    def add_input_wc_pins(self):
        ...
        if self.num_subanks > 1:
            for i in range(self.num_subanks):
                self.add_path("poly",[self.WC_off[i].uc(),(self.WC_off[i].uc().x,-2*i*self.m1_pitch)])
                self.add_contact(self.poly_stack, (self.WC_off[i].lx(),-2*i*self.m1_pitch-self.co_via_shift))

                x_off = self.WC_off[i].lx()+0.5*contact.poly.width
                self.add_path("m1",[(x_off,-2*i*self.m1_pitch), 
                                        (x_off,self.write_comp_off_y- i*self.m2_pitch)])
                self.add_via(self.m1_stack, (self.WC_off[i].lx()+self.co_xshift,
                                             self.write_comp_off_y- i*self.m2_pitch-self.v1_via_shift))

                self.add_rect(layer="m2", 
                              offset=(-self.m1_pitch,
                                       self.write_comp_off_y-i*self.m2_pitch),
                              width=self.width,
                              height = contact.m2m3.first_layer_height)
                ...
                self.add_rect(layer="m2", 
                                    offset=(-self.m1_pitch,self.wc_off_y), 
                                    width=self.dec_en_inst.rx(),
                                    height = contact.m1m2.width)
        ...
        self.add_contact(self.poly_stack, 
                         (self.wack_wc_in_off.lx()-self.via_co_shift,self.wack_inst.uy()-contact.poly.height+self.co_via_shift))
        ...
        pos2= (self.wack_inst.rx()+2*self.m1_pitch, pos1[1])
        pos3= (pos2[0], self.wc_off_y)
        self.add_wire(self.m1_rev_stack, [pos1, pos2, pos3], widen_short_wires=False)
        self.add_via(self.m1_stack, (pos2[0]-0.5*self.m1_width, self.wc_off_y - self.v1_via_shift))
        
        # Add write_complete-gate output
        if self.num_subanks > 1:
            x_off =  self.wc_inst.lx()-self.m1_pitch
            self.add_via(self.m1_stack, (x_off, self.wc_off_y - self.v1_via_shift))
            self.add_wire(self.m1_rev_stack,
                          [self.wc_off.uc(), (self.wc_off.uc().x, self.comp_off_y+0.5*self.m2_width), 
                          (x_off, self.comp_off_y+0.5*self.m2_width),
                          (x_off, self.wc_off.uc().y), (x_off, self.wc_off_y)], widen_short_wires=False)

    def add_input_go_pin(self):
        """ Adds the input go pin """
        for i in range(self.num_subanks):
             self.add_rect(layer="m2", 
                           offset=(-self.m1_pitch,self.go_off_y-i*self.m1_pitch), 
                           width=self.width,
                           height=contact.m1m2.width)
             ...
             self.add_path("poly",[self.wc_go_off[i].uc(),(self.wc_go_off[i].uc().x, -(2*i+1)*self.m1_pitch)])
             self.add_contact(self.poly_stack, (self.wc_go_off[i].lx(),-(2*i+1)*self.m1_pitch-self.co_via_shift))

             x_off = self.wc_go_off[i].lx()+0.5*contact.poly.width
             self.add_path("m1",[(x_off,-(2*i+1)*self.m1_pitch),(x_off, self.go_off_y-i*self.m1_pitch)])
              
             self.add_via(self.m1_stack, (self.wc_go_off[i].lx()+ self.co_xshift,
                                          self.go_off_y-i*self.m1_pitch-self.v1_via_shift))

             self.add_path("poly",[self.dr_go_off[i].uc(),(self.dr_go_off[i].uc().x, -(2*i+1)*self.m1_pitch)])
             self.add_contact(self.poly_stack,(self.dr_go_off[i].lx(),-(2*i+1)*self.m1_pitch-self.co_via_shift))
    ...
    def add_output_ack_pin(self):
        ...
        x_off=self.ack_inst.lx()-4*self.m1_pitch
        if self.power_gate:
            x_off = self.pg_nor[0].lx() - self.m1_pitch
            pos1 = self.pg_nor[0].get_pin("Z").uc()
            pos2 = vector(pos1.x, pos1.y+self.m1_pitch)
            pos3 = vector(x_off, pos2.y)
            pos4 = vector(x_off,self.min_off_y)
            self.add_wire(self.m1_rev_stack, [pos1, pos2, pos3, pos4], widen_short_wires=False)
        
            x1_off=self.ack_inst.lx()-4*self.m1_pitch
            ...
            self.add_wire(self.m1_rev_stack, [pos1, pos2, pos3, pos4, pos5], widen_short_wires=False)
        
        else:
            self.add_wire(self.m1_rev_stack, [self.ack_out_off, 
                                             (x_off, self.ack_out_off.y), (x_off, self.min_off_y)], widen_short_wires=False)
        ...
        self.add_via(self.m1_stack, (x_off-0.5*self.m1_width, self.ack_off_y - self.v1_via_shift))
        self.add_rect(layer="m2", 
        ...
        self.add_path("poly",[self.u_ack_in_off.uc(),(self.u_ack_in_off.uc().x,-self.m1_pitch)])
        self.add_contact(self.poly_stack, (self.u_ack_in_off.lx(),-self.m1_pitch-self.co_via_shift))
        x_off = self.u_ack_in_off.lx()+0.5*contact.poly.width
        self.add_path("m1", [(x_off,-self.m1_pitch), (x_off, self.ack_off_y)])
        self.add_via(self.m1_stack, (self.u_ack_in_off.lx(), self.ack_off_y - self.v1_via_shift))
    ...
    def add_output_wen_pin(self):
        ...
        self.add_rect(layer="m2", 
                      offset=(-self.m1_pitch,self.wen_off_y), 
                      width=self.width,
                      height=contact.m1m2.width)
        ...
        self.add_via(self.m1_stack, (self.wen_out_off.rx()+ self.v1_via_shift,self.wen_off_y), rotate=90)
        self.add_path("m1",[self.wen_out_off.uc(), (self.wen_out_off.uc().x,self.wen_off_y)])

    def add_output_wack_pin(self):
        """ Adds the output wack pin """
        # wack-gate output connection
        x_off = self.wack_inst.rx()+3*self.m1_pitch

        if self.power_gate:
            x_off = self.pg_nor[2].rx() + 4*self.m1_pitch
            ...
            x1_off=self.wack_inst.rx()+3*self.m1_pitch
            ...
            self.add_wire(self.m1_rev_stack, [self.wack_out_off, 
                                             (x_off, self.wack_out_off.y), (x_off, self.min_off_y)], widen_short_wires=False)
        ...
        self.add_via(self.m1_stack, (x_off-0.5*contact.m1m2.width,self.wack_off_y - self.v1_via_shift))
        ...
        self.add_contact(self.poly_stack, (self.ack_wack_in_off1.rx()-contact.poly.width,0-self.co_via_shift))
        self.add_path("m1",[(self.ack_wack_in_off1.rx()-contact.poly.width+0.5*contact.poly.width,0),
                                (self.ack_wack_in_off1.rx()-contact.poly.width+0.5*contact.poly.width,self.wack_off_y)])
        self.add_via(self.m1_stack, (self.ack_wack_in_off1.rx()-contact.poly.width, self.wack_off_y - self.v1_via_shift))
        
        self.add_path("poly",[self.ack_wack_in_off2.uc(), 
                             (self.ack_wack_in_off2.uc().x, self.ack_inst.uy())])
        self.add_contact(self.poly_stack, 
                         (self.ack_wack_in_off2.lx()-self.via_co_shift, self.ack_inst.uy()-contact.poly.height+self.co_via_shift))
        self.add_via(self.m1_stack, (self.ack_wack_in_off2.lx(), 
                                     self.ack_inst.uy()-contact.m1m2.height))
        pos1= (self.ack_wack_in_off2.lx(), self.ack_inst.uy()-0.5*self.m2_width)
        pos2= (self.ack_inst.rx()+self.m1_pitch, pos1[1])
        pos3= (pos2[0], self.wack_off_y)
        self.add_wire(self.m1_rev_stack, [pos1, pos2, pos3], widen_short_wires=False)
        self.add_via(self.m1_stack, (pos2[0], self.wack_off_y - self.v1_via_shift))

    def add_output_pchg_pin(self):
        """ Adds the output pchg pin """

        self.add_rect(layer="m2",
                      offset=(-self.m1_pitch,self.pchg_off_y), 
                      width=self.width,
                      height=contact.m1m2.width)
        ...
        self.add_contact(self.poly_stack, (self.u_pre_pchg_in_off.lx()-self.via_co_shift+self.co_xshift, 0-self.co_via_shift))
        self.add_via(self.m1_stack, (self.u_pre_pchg_in_off.lx()+self.co_xshift, 0))
        
        pos1= (self.u_pre_pchg_in_off.rx(), 0.5*self.m2_width)
        pos2= (self.u_inst.lx()-self.m1_pitch, pos1[1])
        pos3= (pos2[0], pchg_nor3_off.y-contact.m1m2.width)
        pos4= (pchg_nor3_off.x+self.m1_width-self.v1_via_shift, pos3[1])
        self.add_wire(self.m1_rev_stack, [pos1, pos2, pos3, pos4], widen_short_wires=False)
        self.add_via(self.m1_stack,(pos4[0]+contact.m1m2.height,pos3[1]-contact.m1m2.width),rotate=90)
        
        #Route the pchg rail
        self.add_via(self.m1_stack, (self.pchg_off.rx()+ self.v1_via_shift, self.pchg_off_y), rotate=90)
        self.add_path("m1",[self.pchg_off.uc(), (self.pchg_off.uc().x,self.pchg_off_y)])

        # ack-gate pchg-input connection
        self.add_path("poly",[self.ack_pchg_in_off.uc(),
                             (self.ack_pchg_in_off.uc().x,2*contact.poly.height)])
        self.add_contact(self.poly_stack, (self.ack_pchg_in_off.rx()-contact.poly.width-self.via_co_shift, 2*contact.poly.height-self.co_via_shift))
        self.add_via(self.m1_stack, (self.ack_pchg_in_off.rx()-contact.poly.width+0.5*contact.m1m2.width, 2*contact.poly.height-self.co_via_shift), rotate=90)

        
        pos1= (self.ack_pchg_in_off.uc().x, 2*contact.poly.height+0.5*self.m2_width-self.co_via_shift)
        pos2= (self.ack_inst.lx()-2*self.m1_pitch, pos1[1])
        pos3= (pos2[0], self.pchg_off_y)
        self.add_wire(self.m1_rev_stack,[pos1, pos2, pos3], widen_short_wires=False)
        self.add_via(self.m1_stack, (pos2[0]+0.5*self.m1_width+ self.v1_via_shift, self.pchg_off_y), rotate=90)
        
        # decoder_enable_gate pchg-input connection
        self.add_via(self.m1_stack, (self.dec_en_in_off.rx()+ self.v1_via_shift, self.pchg_off_y), rotate=90)
        self.add_path("m1",[self.dec_en_in_off.uc(),(self.dec_en_in_off.uc().x,self.pchg_off_y)])
        ...
        if self.num_subanks > 1:
            ...
            self.add_contact(self.poly_stack, 
                             (self.WC_pchg_off.rx()-self.co_xshift,self.wc_inst.uy()-2*contact.poly.height+self.co_via_shift))
            ...
            self.add_wire(self.m1_rev_stack, [(self.WC_pchg_off.rx(),
                          ...
                          (self.wc_inst.lx()-2*self.m1_pitch,
                          self.wc_inst.uy()-contact.poly.height-0.5*self.m2_width),
                          (self.wc_inst.lx()-2*self.m1_pitch,self.pchg_off_y)], widen_short_wires=False)
            self.add_via(self.m1_stack, (self.wc_inst.lx()-2*self.m1_pitch+0.5*self.m1_width+ self.v1_via_shift,
            ...
            self.add_contact(self.poly_stack, (off.x-self.co_xshift, off.y+self.co_via_shift))
            self.add_via(self.m1_stack, off)

            pos1= (self.DR_pchg_off.rx(), self.dr_inst.uy()-contact.poly.height-0.5*self.m2_width)
            pos2= (self.dr_inst.rx()+self.m1_pitch, pos1[1])
            pos3= (pos2[0], self.pchg_off_y)
            self.add_wire(self.m1_rev_stack, [pos1, pos2, pos3], widen_short_wires=False)
            self.add_via(self.m1_stack, (pos2[0]+0.5*self.m1_width +self.v1_via_shift,self.pchg_off_y), rotate=90)
    ...
    def add_output_rack_pin(self):
        ...
        x_off = self.rack_inst.rx()+2*self.m1_pitch
        if self.power_gate:
            x_off = self.pg_nor[2].rx() + self.m1_pitch 
            pos1 = self.pg_nor[2].get_pin("Z").uc()
            pos2 = vector(pos1.x, pos1.y+2*self.m1_pitch)
            pos3 = vector(x_off, pos2.y)
            pos4 = vector(x_off,self.min_off_y)
            self.add_wire(self.m1_rev_stack, [pos1, pos2, pos3, pos4], widen_short_wires=False)
        
            x1_off=self.rack_inst.rx()+2*self.m1_pitch
            pos1 = self.rack_out_off
            pos2 = vector(x1_off, self.rack_out_off.y)
            pos3 = vector(x1_off, self.dec_en_off_y-self.m2_pitch+0.5*self.m2_width)
            pos5= self.pg_inv[2].get_pin("A").uc()
            pos4 = vector(pos5.x, pos3.y)
            self.add_wire(self.m1_rev_stack, [pos1, pos2, pos3, pos4, pos5], widen_short_wires=False)
        
        else:
            self.add_wire(self.m1_rev_stack, [self.rack_out_off, (x_off, self.rack_out_off.y), (x_off, self.min_off_y)], widen_short_wires=False)

        self.add_layout_pin(text="rack", 
                            layer="m1", 
                            offset=(x_off-0.5*contact.m1m2.width, self.min_off_y), 
                            width=self.m1_width)
        self.add_via(self.m1_stack, (x_off-0.5*self.m1_width,self.rack_off_y - self.v1_via_shift))
        ...
        self.add_contact(self.poly_stack, (self.wen_rack_in_off.rx()+self.poly_to_active-contact.poly.width,
                                           -self.m1_pitch-self.co_via_shift))
        
        x_off =  self.wen_rack_in_off.rx() + self.poly_to_active - 0.5*contact.poly.width
        self.add_path("m1", [(x_off, 0), (x_off, self.rack_off_y)])
        
        self.add_via(self.m1_stack,(x_off-0.5*contact.poly.width, self.rack_off_y - self.v1_via_shift))

        # ack-gate rack-input connection
        self.add_path("poly",[self.ack_rack_in_off.uc(), (self.ack_rack_in_off.uc().x, -self.m1_pitch)])
        self.add_contact(self.poly_stack,(self.ack_rack_in_off.rx()-contact.poly.width, -self.m1_pitch-self.co_via_shift))
        self.add_path("m1",[(self.ack_rack_in_off.rx()-contact.poly.width+0.5*contact.poly.width, -self.m1_pitch), 
                                (self.ack_rack_in_off.rx()-contact.poly.width+0.5*contact.poly.width, self.rack_off_y)])
        self.add_via(self.m1_stack, (self.ack_rack_in_off.rx()-contact.poly.width,self.rack_off_y - self.v1_via_shift))
    ...
    def add_output_dec_en_pin(self):
        ...
        self.add_rect(layer="m2", 
                      ...
                      width=self.width-self.dec_en_out_off.uc().x-self.m1_pitch,
                      height=contact.m1m2.width)
        ...
        self.add_path("m1",[self.dec_en_out_off.uc(),
                               (self.dec_en_out_off.uc().x,self.dec_en_off_y)])
        self.add_via(self.m1_stack, (self.dec_en_out_off.lx(),self.dec_en_off_y - self.v1_via_shift))
    
    def add_U_routing(self):
        """ Routes the u signal """

        self.add_rect(layer="m2", 
                      offset=(-self.m1_pitch, self.U_off_y), 
                      width=self.pin_width, 
                      height = contact.m1m2.width)
        self.add_via(self.m1_stack, (self.u_out_off.uc().x-0.5*contact.m1m2.width,self.U_off_y - self.v1_via_shift))
        self.add_path("m1",[self.u_out_off.uc(), (self.u_out_off.uc().x,self.U_off_y)])
        ...
        self.add_contact(self.poly_stack, (self.wen_u_in_off.lx()+self.poly_to_active-self.via_co_shift+self.co_xshift, 
                                           -self.m1_pitch-self.co_via_shift))
        self.add_via(self.m1_stack, (self.wen_u_in_off.lx()+self.poly_to_active+self.co_xshift, -self.m1_pitch))
        
        pos1= (self.wen_u_in_off.uc().x+self.poly_to_active, -self.m1_pitch+0.5*self.m2_width)
        pos2= (self.wen_inst.lx()-self.m1_pitch,pos1[1])
        pos3= (pos2[0], self.U_off_y)
        self.add_wire(self.m1_rev_stack, [pos1, pos2, pos3], widen_short_wires=False)
        self.add_via(self.m1_stack, (pos2[0]-0.5*self.m2_width, self.U_off_y - self.v1_via_shift))
        ...
        self.add_contact(self.poly_stack, (x_off-self.via_co_shift, -self.m1_pitch-self.co_via_shift))
        self.add_via(self.m1_stack, (x_off, -self.m1_pitch))
        
        pos1= (self.sen_u_in_off.lx()-self.poly_to_active, -self.m1_pitch+0.5*self.m2_width)
        pos2= (self.sen_inst.rx()+self.m1_pitch, pos1[1])
        pos3= (pos2[0], self.U_off_y)
        self.add_wire(self.m1_rev_stack, [pos1, pos2, pos3], widen_short_wires=False)
        self.add_via(self.m1_stack, (pos2[0]-0.5*self.m1_width,self.U_off_y - self.v1_via_shift))
        
        # rack-gate u-input connection
        self.add_path("poly",[self.rack_u_in_off.uc(),
                             (self.rack_u_in_off.uc().x, 2*contact.poly.height)])
        self.add_contact(self.poly_stack, (self.rack_u_in_off.lx()-self.via_co_shift+self.co_xshift, 2*contact.poly.height-self.co_via_shift))
        ...
        pos2=(self.rack_inst.rx()+self.m1_pitch, pos1[1])
        pos3=(pos2[0], self.U_off_y)
        self.add_wire(self.m1_rev_stack, [pos1, pos2, pos3], widen_short_wires=False)
        self.add_via(self.m1_stack, (pos2[0]-0.5*self.m1_width, self.U_off_y - self.v1_via_shift))
        ...
        self.add_path("poly",[self.wack_u_in_off.uc(),
                             (self.wack_u_in_off.uc().x,-self.m1_pitch)])
        self.add_contact(self.poly_stack,(self.wack_u_in_off.lx()-self.via_co_shift, -self.m1_pitch-self.co_via_shift))
        self.add_via(self.m1_stack, (self.wack_u_in_off.lx(), -self.m1_pitch))

        pos1= (self.wack_u_in_off.uc().x,-self.m1_pitch+0.5*self.m2_width)
        pos2= (self.wack_inst.lx()-self.m1_pitch,pos1[1])
        pos3= (pos2[0], self.U_off_y)
        self.add_wire(self.m1_rev_stack, [pos1, pos2, pos3], widen_short_wires=False)
        self.add_via(self.m1_stack, (pos2[0]-0.5*self.m1_width, self.U_off_y - self.v1_via_shift))
    ...
    def add_output_sen_pin(self):
        ...
        self.add_rect(layer="m2", 
                      offset=(-self.m1_pitch,self.sen_off_y), 
                      width=self.width,
                      height=contact.m1m2.width)
        ...
        pos2= vector(pos1.x, pos1.y+2*self.m1_pitch)
        pos3= vector(self.pin_width+2*self.m1_pitch, pos2.y)
    ...
    def add_dc_routing(self):
        ...
        pos2= vector(pos1.x, self.dc_inst.by()-self.m1_pitch)
        pos3= vector(self.xsen_in_off.x, pos2.y)
        pos4 = self.xsen_in_off
        if abs(pos1.x-pos3.x)<self.m1_pitch:
    ...
    def route_vdd_gnd(self):
        ...
        self.add_rect(layer="m2",
                      offset=(-self.m1_pitch, self.vdd_off_y), 
                      width=self.width,
                      height=size)
        ...
        self.add_rect(layer="m2", 
                      offset=(-self.m1_pitch, self.gnd_off_y), 
                      width=self.width,
                      height=size)
        ...
        for i in range(2):
            pos1= self.dc_inst.get_pin(pins[i]).lc()-vector(0, (i+2)*self.m1_pitch+0.5*contact.m1m2.width)
            pos2=vector(self.pin_width-(i+1)*self.m1_pitch, pos1.y)
            pos3=vector(pos2.x, off[i])
            self.add_wire(self.m1_rev_stack,[pos1, pos2, pos3], widen_short_wires=False)
            self.add_via(self.m1_stack,(pos3.x-0.5*contact.m1m2.width, off[i]-self.v1_via_shift))
        ...
        for inst in gates_list:
            for gnd_pin in inst.get_pins("gnd"):
                ...
                self.add_via(self.m1_stack, (gnd_pin.uc().x-0.5*contact.m1m2.width, self.gnd_off_y-self.v1_via_shift))
            ...
            for vdd_pin in inst.get_pins("vdd"):
                ...
                self.add_via(self.m1_stack, (vdd_pin.uc().x-0.5*contact.m1m2.width, self.vdd_off_y-self.v1_via_shift))

        if info["has_nimplant"]:
            if self.num_subanks > 1:
                off = (2*self.num_subanks-1)*self.m1_pitch
            else:
                off = 3*self.m1_pitch
