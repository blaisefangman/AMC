...
from async_bank import bank
from async_split_array import split_array
from async_merge_array import merge_array
from async_split_merge_control import split_merge_control
...
class multi_bank(design.design):
        ...
        if self.num_subanks>1:
            for i in range(self.w_size):
                ...
                for j in range(self.num_subanks):
                    for (pin, stack) in zip(pin_list, stack_list):
                        ...
                        self.add_via(stack,(offset.x, offset.y-self.v1_via_shift))
            ...
            for i in range(self.w_size):
                ...
                for j in range(self.num_subanks):
                    ...
                    self.add_via(self.m1_stack,(offset.x, offset.y-self.v1_via_shift))
    ...
    def add_busses(self):
        ...
        if self.power_gate:
            self.v_ctrl_bus_pos= self.create_bus(layer="m2",
                                                 pitch=self.pitch,
            ...
            self.v_ctrl_bus_pos.update(self.create_bus(layer="m2",
                                                       pitch=self.pitch,
        ...
        self.din1_bus_pos= self.create_bus(layer="m1",
                                           pitch=self.pitch,
                                           ...
                                           make_pins=make_pin)
        if self.mask:
            self.bm_bus_pos= self.create_bus(layer="m3",
                            pitch=self.pitch,
                            offset=self.din1_bus_off,
                            names=bm_bus_names,
                            length=self.data_bus_width,
        ...
        self.dout1_bus_pos= self.create_bus(layer="m1",
                                            pitch=self.pitch,
        ...
        if (self.num_banks == 4 and  self.orien == "V"):
            ...
            self.din2_bus_pos= self.create_bus(layer="m1",
                                               pitch=self.pitch,
            ...
            if self.two_level_bank:
                ...
                dout_bus_names=["dout[{0}]".format(i) for i in range(self.w_size)]
            self.dout2_bus_pos= self.create_bus(layer="m1",
                                                pitch=self.pitch,
    ...
    def route_banks(self):
                ...
                for i in range(self.num_subanks):
                  for j in range(self.w_size):
                      ...
                      self.add_rect(layer="m2", 
                                    ...
                                    width=self.m2_width, 
                                    height=din_height)
                      self.add_via(self.m1_stack, (din_off.x, din_off.y-self.v1_via_shift))
                      ...
                      if self.mask:
                          ...
                          self.add_rect(layer="m2", 
                                        ...
                                        width=self.m2_width, 
                                        height=din_height)
                          self.add_via(self.m2_stack, (bm_off.x, bm_off.y-self.v1_via_shift))
                      ...
                      self.add_rect(layer="m2", 
                                    ...
                                    width=self.m2_width, 
                                    height=dout_height)
                      self.add_via(self.m1_stack, (dout_off.x, dout_off.y-self.v1_via_shift))
        ...
        if (self.num_banks == 4 and  self.orien == "V"):
            ...
            for j in range(self.w_size):                      
                ...
                self.add_wire(self.m1_stack, [doutoff1, doutoff2, doutoff3, doutoff4], widen_short_wires=False)
                ...
                self.add_wire(self.m1_stack, [dinoff1, dinoff2, dinoff3, dinoff4], widen_short_wires=False)
                ...
                if self.mask:
                    ...
                    self.add_wire(self.m2_rev_stack, [bmoff1, bmoff2, bmoff3, bmoff4], widen_short_wires=False)
        ...
        for k in range(self.num_banks):
            for i in range(self.bank_addr_size):
                ...
                addr_off= vector(self.addr_bus1_off.x+ i*self.pitch- self.m3_enclose_via2, 
                ...
                self.add_rect(layer=layer, 
                              ...
                              width=addr_width, 
                              height=height)
                self.add_via(stack, (addr_off.x, addr_off.y-self.v1_via_shift))
        ...
        for k in range(self.num_banks):
            ...
            self.add_rect(layer=layer, 
                          ...
                          height=height)
            self.add_via(stack, (bank_sel_off.x, bank_sel_off.y-self.v1_via_shift))
        ...
        for k in range(self.num_banks):
            ...
            for i in range(len(control_pin_list)):
                ...
                self.add_rect(layer=layer, 
                              ...
                              width=ctrl_width, 
                              height=height)
                self.add_via(stack, (ctrl_off.x, ctrl_off.y-self.v1_via_shift))             
        ...
        if not self.two_level_bank:
            ...
            self.add_path("m1", [sel_pos1, sel_pos2])
            self.add_via(self.m1_stack, (sel_pos1.x-0.5*self.m2_width, 
                         sel_pos1.y-0.5*self.m1_width-self.v1_via_shift))
            self.add_via(self.m1_stack, (sel_pos2.x-0.5*self.m2_width, 
                         sel_pos2.y-0.5*self.m1_width-self.v1_via_shift))
        ...
        if (self.num_banks == 4 and  self.orien == "V"):
            self.add_via(self.m2_stack, (self.vdd_off.x-self.v2_via_shift, self.pow_rail_2_off.y+\
                        0.5*self.m1_width-self.v1_via_shift), size=[1, self.num_via])
            self.add_via(self.m2_stack, 
                        (self.gnd_off.x-self.v2_via_shift,self.pow_rail_2_off.y+self.pow_pitch+\
                         0.5*self.m1_width-self.v1_via_shift), size=[1, self.num_via])
...
#/////////////////////////////////////////////////////////////////////////////#
    ...
    def route_data_split_merge(self):
        ...
        if (self.num_banks == 2 and self.orien == "H"):
            ...
            for i in range(self.w_size):
                ...
                self.add_wire(self.m1_stack, 
                              ...
                              (self.dsplitoff[i].x, self.dsplitoff[i].y+0.5*self.m1_width)], widen_short_wires=False) 
                if self.mask:
                    ...
                    self.add_wire(self.m2_rev_stack, 
                                  ...
                                  (self.dsplitoff[i].x, self.dsplitoff[i].y+0.5*self.m3_width)], widen_short_wires=False) 
        ...
        for i in range(self.w_size):
            ...
            self.add_path("m2", [din_pos, mid_pos, (din_pos.x, y_off)])
            ...
            if self.mask:
                ...
                self.add_path("m2", [bm_pos, mid_pos, (bm_pos.x, y_off)])
                if (self.num_banks==4 or self.orien == "V"):
                    self.add_via_center(self.m2_stack, (bm_pos.x, y_off))
            ...
            if self.num_banks == 4:
                for i in range(self.w_size):
                    ...
                    self.add_wire(self.m1_stack, 
                                  ...
                                  (self.dmrgoff[i].x, 
                                   self.dmrgoff[i].y+0.5*self.m1_width)], widen_short_wires=False) 
        ...
        if self.orien == "V":
            ...
            for i in range(self.w_size):
                ...
                self.add_wire(self.m1_stack, [(data_out_pos_x, data_out_pos_y),
                             (data_out_pos_x+(i+1)*self.pitch, data_out_pos_y),
                             (data_out_pos_x+(i+1)*self.pitch, self.dmrgoff[i].y),
                             (self.dmrgoff[i].x,self.dmrgoff[i].y+0.5*self.m1_width)], widen_short_wires=False) 
        ...
        for i in range(self.w_size):
            ...
            self.add_path("m2", [pos1, mid_pos1, mid_pos2, pos2], width=contact.m1m2.width)
            if (self.num_banks == 4 or self.orien=="V"):
    ...
    def route_addr_ctrl_split_merge(self):
        ...
        for i in range(self.addr_size):
            ...
            self.add_path("m3", [addr_split_pos,(addr_split_pos[0],addr_split_y_off),
            ...
            self.add_via(self.m2_stack,(addr_bus_pos[0]-0.5*self.m3_width,
                                        addr_split_y_off-0.5*self.m3_width-self.v2_via_shift))
            self.add_path("m2",[(addr_bus_pos[0],addr_split_y_off),addr_bus_pos ])
        ...
        for i in range(self.control_size):
            ...
            self.add_wire(self.m1_stack, [ctrl_pos[i]-vector(0, self.m2_width), (ctrl_pos[i].x,ctrl_split_y_off),
                          (ctrl_bus_pos[0],ctrl_split_y_off), ctrl_bus_pos], widen_short_wires=False) 
    ...
    def route_split_cells_powers_and_selects(self):
        ...
        for i in range(3):
            for mod in (mod_list1 + mod_list2):
                ...
                self.add_wire(self.m1_stack,[power_pos.lc(), 
                              ...
                              (power_pos.lx()-(1+i)*self.pitch, y_off-(i+1)*self.pitch)], widen_short_wires=False)
        ...
        for mod in mod_list1:
            ...
            self.add_wire(self.m1_stack, [select_pos.lc(),
                          ...
                          (select_pos.lx()-4*self.pitch, y_off-4*self.pitch)], widen_short_wires=False)
        ...
        self.add_wire(self.m1_stack, [pos1, pos2, pos3, pos4, ctrl_bus_pos], widen_short_wires=False)
        ...
        for i in range(4):
            ...
            if self.orien == "H":
                ...
                self.add_wire(self.m1_stack, [select_pos.lc(), 
                              (self.dmerge_ary_inst.rx()+(1+i)*self.pitch, 
                                select_pos.lc().y), 
                              (self.dmerge_ary_inst.rx()+(1+i)*self.pitch, 
                                y_off-(1+i)*self.pitch)], widen_short_wires=False)
            ...
            if self.orien == "V":
                ...
                self.add_wire(self.m1_stack, [select_pos.lc(), 
                              ...
                              (select_pos.lx(), y_off-(1+i)*self.pitch+0.5*self.m1_width)], widen_short_wires=False)
        ...
        for i in range(2):
            ...
            self.add_wire(self.m1_stack, [(spl_mrg_power.lx(), spl_mrg_power.lc().y),
                         ...
                         (bank_power.uc().x+0.5*self.m2_width, self.reset_off.y+self.m2_width)], widen_short_wires=False)
        ...
        self.add_wire(self.m2_rev_stack, [sel_pos0, sel_pos1, sel_pos2, sel_pos3], widen_short_wires=False)
        ...
        self.add_wire(self.m1_stack, [(spl_mrg_reset.lx(), spl_mrg_reset.lc().y),
                     ...
                     (self.reset_off.x+0.5*self.m2_width, self.reset_off.y+self.pitch)], widen_short_wires=False)
        ...
        self.add_wire(self.m1_stack, [(spl_mrg_ack_merge.x, spl_mrg_ack_merge.y),
                     ...
                     (self.reset_off.x-6*self.pitch, 
                      self.ctrl_split_ary_inst.by()-8*self.pitch)], widen_short_wires=False)
        ...
        for i in range(2):
            ...
            self.add_wire(self.m1_stack,
                          ...
                          (en_data.lx()-(i+5)*self.pitch, en_data.lc().y), en_data.lc()], widen_short_wires=False)

            self.add_wire(self.m1_stack,
                          [en_addr.lc(), (en_addr.lx()-(i+5)*self.pitch, en_addr.lc().y), 
                          (en_addr.lx()-(i+5)*self.pitch, yoff), 
                          (en_ctrl.lx()-(i+5)*self.pitch, yoff),
                          (en_ctrl.lx()-(i+5)*self.pitch, en_ctrl.lc().y), en_ctrl.lc()], widen_short_wires=False)
        ...
        if self.orien == "H":
            self.add_wire(self.m1_stack,
                          ...
                          (self.dmerge_ary_inst.rx()+5*self.pitch, en_data.lc().y), 
                           en_data.lc()], widen_short_wires=False)
        ...
        if self.orien == "V":
            ...
            self.add_wire(self.m1_stack,
                          ...
                          (x_off+index*self.pitch, 
                           en_data.lc().y), en_data.lc()], widen_short_wires=False)
