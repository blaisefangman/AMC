...
class bank_control_logic(design.design):
    ...
    def create_layout(self):
        ...
        if self.num_subanks > 1:
            ...
            self.pin_width = self.dr_inst.rx() + 4*self.metal1_pitch
        else:
            if self.two_level_bank:
                # rreq_merge_inv_inst + 3*m1_pitch for connections
                self.pin_width = self.rreq_mrg_inv1_inst.rx() + 3*self.metal1_pitch
            else:
                # decoder_enable_inst + 1*m1_pitch for connections
                self.pin_width = self.dec_en_inst.rx() + self.metal1_pitch
        
        if self.power_gate:
            self.pin_width = self.pin_width+3*self.inv.height+4*self.metal1_pitch+2*self.gap

        self.dc = delay_chain(num_inv=self.num_inv, num_stage=self.num_stage)
        self.add_mod(self.dc)
        self.add_dc(0)

        self.height= self.dc_inst.uy() - self.min_off_y + 3*self.metal1_pitch

        #delay chain inst + + 5*m1_pitch for connections
        self.pin_width = max(self.dc.height+5*self.metal1_pitch, self.pin_width) 
        
        if self.power_gate:
            self.add_power_gate_cells()
        
        #total width= pin_width + 3*m1_pitch for spaces for both sides
        self.width = self.pin_width + 3*self.metal1_pitch
    ...
    def setup_layout_offsets(self):
        ...
        self.gap= max(self.implant_space, self.well_space, self.metal1_pitch)
        ...
        self.wen_off_y = v_shift+2*self.metal2_pitch
        self.pchg_off_y = self.wen_off_y+self.metal2_pitch
        self.reset_bar_off_y = self.pchg_off_y+self.metal2_pitch
        if self.num_subanks > 1:
            self.comp_off_y = self.reset_bar_off_y            
        self.xsen_off_y = self.U_off_y = self.reset_bar_off_y+self.metal2_pitch
        self.sen_off_y = self.U_off_y+self.metal2_pitch
        self.vdd_off_y = self.sen_off_y+self.metal2_pitch
        self.gnd_off_y = self.vdd_off_y+self.metal2_pitch
        
        # Y-offset of input/output pins blow the gates
        self.r_off_y = -2*self.metal2_pitch
        self.wc_off_y = self.r_off_y-self.metal2_pitch
        self.rw_off_y = self.wc_off_y-self.metal2_pitch
        self.dr_off_y = self.rw_off_y-self.metal2_pitch
        self.ack_off_y = self.dr_off_y-self.metal2_pitch
        self.wack_off_y = self.ack_off_y-self.metal2_pitch
        self.rreq_off_y = self.wack_off_y-self.metal2_pitch
        self.data_ready_off_y = self.rreq_off_y-self.metal2_pitch
        self.w_off_y = self.data_ready_off_y-self.num_subanks*self.metal2_pitch
        self.write_comp_off_y = self.w_off_y-self.metal2_pitch
        self.rack_off_y = self.write_comp_off_y-self.num_subanks*self.metal2_pitch
        self.reset_off_y = self.rack_off_y
        self.go_off_y = self.reset_off_y-self.metal2_pitch
        self.dec_en_off_y =self.go_off_y-(self.num_subanks+1)*self.metal2_pitch
        self.min_off_y = self.dec_en_off_y - 2*self.metal2_pitch
        if self.power_gate:
            self.sleep_off_y = self.min_off_y - self.metal2_pitch
            self.min_off_y = self.sleep_off_y - 2*self.metal2_pitch            
    ...
    def add_modules(self):
        ...
        if self.two_level_bank:
            ...
            self.rreq_mrg_inv1_inst_lr_x=self.wack_inst.rx()+2*self.inv.height+2*self.metal1_pitch
        if self.num_subanks > 1: 
            self.add_WC_OR_gate()
            self.add_DR_OR_gate()
    ...
    def add_m1_minarea(self, pin, width=contact.m1m2.first_layer_width):
        ...
        self.add_rect_center(layer="metal1",
                             offset=pin,
                             width=width,
                             height=self.minarea_metal1/width)
    ...
    def add_Zwire(self, pin1, pin2, gap):
        ...
        self.add_wire(self.metal1_rev_stack,[pin1.uc(), (pin1.uc().x, pin1.lc().y-gap), 
                                        (pin2.uc().x, pin1.lc().y-gap), pin2.uc()])
    ...
    def add_u_gate(self):
        ...
        u_off= (self.pchg_inv_inst.rx() + self.u_gate.height + 2*self.metal1_pitch, 0)
        ...
        self.add_Zpath(self.u_inst.get_pin("Dn0"), self.u_inst.get_pin("Dp1"), self.metal1_pitch)
        ...
        self.add_m1_minarea(self.u_inst.get_pin("Dn1").cc()+vector(0, self.metal1_width))
        ...
        self.add_m1P_minarea(vector(self.u_inst.get_pin("Gp0").lc().x+self.co_xshift,self.u_inst.get_pin("Gp0").lc().y), 
                             self.u_inst.by(),0)
        self.add_m1P_minarea(self.u_inst.get_pin("Gn0").lr(), self.u_inst.uy()+self.metal1_pitch, 0) 

    def add_wen_gate(self):
        """ Adds the pull_up_pull_down network for wen-gate """
        
        #add 3*m1_pitch for 3 connections of wen_gate
        wen_off= (self.u_inst.rx()+3*self.metal1_pitch,0)
        ...
        #Adding a Z connection for output node
        self.add_Zpath(self.wen_inst.get_pin("Dn0"),self.wen_inst.get_pin("Dp1"),self.metal1_pitch) 

        #Connecting the output node to inv input
        self.add_Zpath(self.wen_inv_inst.get_pin("A"),self.wen_inst.get_pin("Dn0"),self.metal1_pitch)
        ...
        self.add_Zwire(self.wen_inst.get_pin("Sn0"), self.wen_inst.get_pin("Dn2"),self.metal1_pitch)
        ...
        self.add_m1_minarea(self.wen_inst.get_pin("Dn1").cc()+vector(0, self.metal1_width))
        self.add_m1_minarea(self.wen_inst.get_pin("Dn3").cc()+vector(0, self.metal1_width))
    ...
    def add_sen_gate(self):
        ...
        #Adding a Z connection for output node
        self.add_Zpath(self.sen_inst.get_pin("Dp1"),self.sen_inst.get_pin("Dn0"),-self.metal1_pitch)
        
        #Connecting the output node to inv input
        self.add_Zpath(self.sen_inv_inst.get_pin("A"),self.sen_inst.get_pin("Dn0"),self.metal1_pitch)
        ...
        self.add_via_center(self.metal1_stack, (pos1.x, pos1.y+gap))
        self.add_via_center(self.metal1_stack, (pos2.x, pos2.y+gap))
        ...
        #Adding min_area metal1 for active contacts with no connections and poly_contact positions
        self.add_m1_minarea(self.sen_inst.get_pin("Dn2").cc()+vector(0, self.metal1_width))
        self.add_m1_minarea(self.sen_inst.get_pin("Dp0").cc()-vector(0, self.metal1_width))
    ...
    def add_ack_gate(self):
        ...
        ack_off= (self.sen_inst.rx()+6*self.metal1_pitch,0)
        ...
        self.ack_out_off=self.ack_inv_inst.get_pin("Z").lr()-vector(0,0.5*self.metal2_width)
        ...
        #Adding a Z connection for active contact connections
        self.add_Zwire(self.ack_inst.get_pin("Dn0"),self.ack_inst.get_pin("Dn4"),-self.metal2_pitch)
        self.add_Zpath(self.ack_inst.get_pin("Dn3"),self.ack_inst.get_pin("Dn5"),self.metal1_pitch) 

        #Adding a Z connection for output node
        self.add_Zpath(self.ack_inst.get_pin("Sn0"), self.ack_inst.get_pin("Dp2"),self.metal1_pitch) 
        
        #Connecting output of ack_inst to inv input
        self.add_path("metal1",[self.ack_inst.get_pin("Sn0").uc(),
                               (self.ack_inv_inst1.get_pin("A").lr()-vector(0,0.5*self.metal1_width))])
        ...
        self.add_m1_minarea(self.ack_inst.get_pin("Dn1").cc()+vector(0, self.metal1_width))
        self.add_m1_minarea((self.ack_inst.get_pin("Dn2").uc() + vector(0, self.metal1_width)))
        self.add_m1P_minarea(vector(self.ack_inst.get_pin("Gn0").rx()-contact.poly.width, self.ack_inst.get_pin("Gn0").by()), 
                             self.ack_inst.by(),-self.metal1_pitch, self.metal1_width)
        self.add_m1P_minarea(vector(self.ack_inst.get_pin("Gn3").rx()+self.co_xshift-contact.poly.width, self.ack_inst.get_pin("Gn3").lc().y),
                             self.ack_inst.by(),2*self.metal1_pitch)
        self.add_m1P_minarea(self.ack_inst.get_pin("Gn1").lc(),self.ack_inst.uy(),-self.metal1_width)
        self.add_m1P_minarea(self.ack_inst.get_pin("Gn3").lc(),self.ack_inst.uy(),-self.metal1_width)
        
    def add_rack_gate(self):
        """" Adds the pull_up_pull_down network for rack-gate """

        #add 4*m1_pitch for 4 connections of rack_gate
        rack_off= (self.ack_inst.rx()+self.rack_gate.height+4*self.metal1_pitch,0)
        ...
        # -0.5*con.m1m2.width for vdd sharing
        rack_inv_off1= (rack_off[0]-0.5*contact.m1m2.width, self.rack_gate.width+self.gap+self.metal1_space)        
        ...
        self.rack_out_off=self.rack_inv_inst.get_pin("Z").bc()-vector(0,0.5*self.metal2_width)
        ...
        #Adding a Z connection for output node
        self.add_Zpath(self.rack_inst.get_pin("Dn2"),self.rack_inst.get_pin("Dp1"),self.metal1_pitch+self.metal1_space)

        #Connecting output of rack_inst to inv input
        self.add_Zpath(self.rack_inst.get_pin("Dn2"),self.rack_inv_inst1.get_pin("A"),-self.metal1_pitch-self.metal1_width)
        ...
        self.add_rect(layer="pimplant",
                      ...
                      height=self.gap+self.metal1_space)
        ...
        self.add_m1_minarea(self.rack_inst.get_pin("Dn0").cc()-vector(0, self.metal1_width))
        ...
        self.add_m1P_minarea(self.rack_inst.get_pin("Gn2").lc(), self.rack_inst.uy(),-self.metal1_width)
        
    def add_wack_gate(self):
        """" Adds the pull_up_pull_down network for wack-gate """

        #add 5*m1_pitch for 5 connections of wack_gate
        wack_off= (self.rack_inst.rx()+5*self.metal1_pitch,0)
        ...
        self.wack_out_off=self.wack_inv_inst.get_pin("Z").bc()-vector(0,self.metal2_width)
        ...
        #Adding a Z connection for output node
        self.add_Zpath(self.wack_inst.get_pin("Dn2"),self.wack_inst.get_pin("Dp1"),self.metal1_pitch)
        
        #Connecting output of wack_inst to inv input
        pos1=(self.wack_inv_inst1.get_pin("A").bc()-vector(0,0.5*self.metal1_width))
        pos2=(self.wack_inv_inst1.rx()-self.metal1_space-contact.m1m2.width,pos1[1])
        ...
        self.add_m1_minarea(self.wack_inst.get_pin("Dn1").cc()+vector(0, self.metal1_width))
    ...
    def add_dec_en_gate(self):
        ...
        delay_inv_off= (self.wack_inst.rx()+4*self.metal1_pitch+\
    ...
    def add_rreq_merge_enable(self):
        ...
        rreq_mrg_inv_off= (self.wack_inst.rx()+ 2* self.inv.height+4*self.metal1_pitch+\
                           0.5*contact.m1m2.width,self.inv5.width)        
        ...
        self.rreq_mrg_inv2_inst=self.add_inst(name="rreq_merge_inv_1", 
                                              mod=self.inv, 
                                              offset=rreq_mrg_inv_off+vector(0,self.inv.width+3*self.metal1_pitch), 
                                              mirror="MX", 
                                              rotate=270)
        ...
        mid_pos1=(rreq_mrg_in_off1.x, rreq_mrg_in_off1.y+self.metal1_pitch)
        mid_pos2=(rreq_mrg_out_off1.x, rreq_mrg_in_off1.y+self.metal1_pitch)
        ...
        pos1=rreq_mrg_in_off+vector(0,self.metal1_pitch)
        pos2=(self.rreq_mrg_inv2_inst.rx()+self.metal1_pitch,pos1[1])
        pos3=(pos2[0], self.rreq_off_y)
        self.add_wire(self.metal1_rev_stack, [rreq_mrg_in_off,pos1, pos2, pos3]) 
        
        self.add_via(self.metal1_stack,(pos2[0],self.rreq_off_y), rotate=90)
                           
        # Calculating the offset for next cell
        self.rreq_mrg_inv1_inst_lr_x = self.rreq_mrg_inv1_inst.rx()+2*self.metal1_pitch
    ...
    def add_WC_OR_gate(self):
        ...
        wc_off= (self.rreq_mrg_inv1_inst_lr_x+self.wc_gate.height+2*self.metal1_pitch,
                 self.wc_gate.width)
        ...
        for i in range(self.num_subanks//2 - 1):
            self.add_Zpath(Dn_contact[i], Dn_contact[i+1], -1.5*self.metal1_pitch)
        ...
        self.add_Zpath(self.wc_inv_inst.get_pin("A"),Dp_contact,self.metal1_pitch)
        ...
        self.add_m1P_minarea(self.wc_inst.get_pin("Gp0").lr(),self.wc_inst.uy(),self.metal1_pitch)
    ...
    def add_DR_OR_gate(self):
        ...
        self.add_Zpath(self.dr_inv_inst.get_pin("A"), Dp_contact, self.metal1_pitch)
        ...
        self.add_m1P_minarea(self.dr_inst.get_pin("Gp0").lr(),self.dr_inst.uy(),self.metal1_pitch)
        for i in range(2*self.num_subanks-1):
    ...
    def add_dc(self,y_off):
        ...
        dc_off= (self.dc.height, self.gnd_off_y+3*self.metal1_pitch)
    ...
    def add_power_gate_cells(self):
        ...
        if self.num_subanks > 1:
            xoff = self.dr_inst.rx() + 2*self.metal1_pitch
        ...
        for i in range(3):
            ...
            self.add_path("metal1", [(pin1.uc().x, pin1.uy()-self.metal1_width), (pin2.uc().x, pin2.by()+self.metal1_width)])
    ...
    def add_input_rw_pin(self):
        ...
        self.add_via(self.metal1_stack,(pchg_B.x, self.rw_off_y - self.via_shift("v1")))
        ...
        self.add_via(self.metal1_stack,(self.wen_rw_in_off.rx()-contact.poly.width, self.rw_off_y - self.via_shift("v1")))
        ...
        self.add_via(self.metal1_stack, (self.sen_rw_in_off.lx(), self.rw_off_y - self.via_shift("v1")))
        ...
        pos4=(pos3[0], -self.metal1_pitch)
        self.add_path("poly", [pos1, pos2, pos3, pos4])
        self.add_contact(self.poly_stack,(self.ack_rw_in_off.lx()+self.poly_to_active, -self.metal1_pitch-self.via_shift("co")))
        self.add_path("metal1",[(self.ack_rw_in_off.lx()+0.5*contact.poly.width+self.poly_to_active,-self.metal1_pitch), 
                                (self.ack_rw_in_off.lx()+0.5*contact.poly.width+self.poly_to_active,self.rw_off_y)])
        self.add_via(self.metal1_stack, (self.ack_rw_in_off.lx()+self.poly_to_active, self.rw_off_y - self.via_shift("v1")))
    ...
    def add_input_w_pin(self):
        ...
        self.add_via(self.metal1_stack, (pchg_C.x, self.w_off_y - self.via_shift("v1")))
        ...
        self.add_via(self.metal1_stack, (self.wen_w_in_off.rx()-contact.poly.width, self.w_off_y - self.via_shift("v1")))
        ...
        pos4=(pos3[0], -3*self.metal1_pitch)
        self.add_path("poly", [pos1, pos2, pos3, pos4])
        self.add_contact(self.poly_stack, (self.ack_w_in_off.rx()+self.poly_to_active-contact.poly.width, -3*self.metal1_pitch-self.via_shift("co")))
        
        pos1=(self.ack_w_in_off.rx()+self.poly_to_active+0.5*contact.poly.width-contact.poly.width,-3*self.metal1_pitch)
        pos2=(pos1[0],self.w_off_y)
        self.add_path("metal1",[pos1, pos2])
        self.add_via(self.metal1_stack, (self.ack_w_in_off.rx()+self.poly_to_active-contact.poly.width, self.w_off_y - self.via_shift("v1")))
    ...
    def add_input_r_pin(self):
        ...
        self.add_rect(layer="metal2",
                      ...
                      width=self.ack_inst.lx()-3*self.metal1_pitch-pchg_A.x,
                      height = contact.m1m2.width)
        self.add_via(self.metal1_stack, (pchg_A.x, self.r_off_y - self.via_shift("v1")))
        ...
        self.add_via(self.metal1_stack, (self.sen_r_in_off.rx()+contact.m1m2.height, self.r_off_y), rotate=90)
        ...
        self.add_via(self.metal1_stack, (self.ack_r_in_off.lx(), 
                                     self.ack_inst.uy()-contact.m1m2.height))

        pos1=(self.ack_r_in_off.uc().x, self.ack_inst.uy()-0.5*self.metal2_width)
        pos2=(self.ack_inst.lx()-3*self.metal1_pitch, pos1[1])
        pos3=(pos2[0], self.r_off_y)
        self.add_wire(self.metal1_rev_stack, [pos1, pos2, pos3])
        self.add_via(self.metal1_stack, (pos2[0]+0.5*self.metal1_width+ self.via_shift("v1"),self.r_off_y), rotate=90)
    ...
    def add_sleep_pin(self):
        ...
        pos1=(self.pg_nor[0].get_pin("B").lx(),self.sleep_off_y-self.metal2_pitch)
        pos2=(self.pg_nor[2].get_pin("B").lx(),self.sleep_off_y-self.metal2_pitch)
        ...
        for i in range(3):
            ...
            pos2=(pos.uc().x, self.sleep_off_y-self.metal2_pitch)
            self.add_path("metal1",[pos1 , pos2], width=contact.m1m2.width)
            self.add_path("metal2",[(pos.uc().x, pos.by()) , (pos.uc().x, self.pg_inv[0].by())])
            self.add_via_center(self.metal1_stack, pos2)
            self.add_via_center(self.metal1_stack, pos1)
            self.add_via_center(self.metal1_stack, (pos.uc().x, pos.by()))
    ...
    def add_input_reset_pin(self):
        ...
        self.add_rect(layer="metal2", 
                      ...
                      width=self.u_inst.rx()+self.metal1_pitch-self.rst_in_off.uc().x,
                      height = contact.m1m2.width)
        self.add_via(self.metal1_stack, (self.rst_in_off.uc().x,self.reset_off_y - self.via_shift("v1")))
        self.add_path("poly",[self.u_rst_in_off.uc(), 
                             (self.u_rst_in_off.uc().x,self.u_gate.width+contact.poly.height+self.metal1_pitch)])
        self.add_contact(self.poly_stack, 
                         (self.u_rst_in_off.rx()-self.via_co_shift, self.u_gate.width+self.metal1_pitch+self.via_shift("co")))
        self.add_via(self.metal1_stack, (self.u_rst_in_off.rx(), self.u_gate.width+self.metal1_pitch))
        
        pos1=(self.u_rst_in_off.rx(), self.u_gate.width+self.metal1_pitch+0.5*self.metal2_width)
        pos2=(self.u_inst.rx()+self.metal1_pitch, pos1[1])
        pos3=(pos2[0], self.reset_off_y)
        self.add_wire(self.metal1_rev_stack, [pos1, pos2, pos3])
        self.add_via(self.metal1_stack, (pos2[0], self.reset_off_y - self.via_shift("v1")))

        # reset-gate reset_bar connection
        self.add_rect(layer="metal2", 
                      offset=(self.rst_out_off.uc().x,self.reset_bar_off_y), 
                      width=self.wack_inst.rx()+self.metal1_pitch-self.rst_out_off.uc().x,
                      height = contact.m1m2.width)

        self.add_via(self.metal1_stack, (self.rst_out_off.lx(),self.reset_bar_off_y - self.via_shift("v1")))
        self.add_path("metal1",[self.rst_out_off.uc(),
                               (self.rst_out_off.uc().x,self.reset_bar_off_y)])

        # ack-gate reset_bar connection
        self.add_path("poly",[self.ack_reset_bar_in_off.uc(), (self.ack_reset_bar_in_off.uc().x, -2*self.metal1_pitch)])
        self.add_contact(self.poly_stack, (self.ack_reset_bar_in_off.rx()-self.via_co_shift+self.co_xshift-contact.poly.width, -2*self.metal1_pitch-self.via_shift("co")))
        self.add_via(self.metal1_stack, (self.ack_reset_bar_in_off.rx()+self.co_xshift-contact.poly.width, -2*self.metal1_pitch))

        pos1=(self.ack_reset_bar_in_off.uc().x,-2*self.metal1_pitch+0.5*self.metal2_width)
        pos2=(self.ack_inst.lx()-self.metal1_pitch, pos1[1])
        pos3=(pos2[0], self.reset_bar_off_y)
        self.add_wire(self.metal1_rev_stack, [pos1, pos2, pos3])
        self.add_via(self.metal1_stack, (pos2[0]-0.5*self.metal1_width, self.reset_bar_off_y - self.via_shift("v1")))

        # rack-gate reset_bar connection
        self.add_path("poly",[self.rack_rst_b_in_off.uc(), 
                             (self.rack_rst_b_in_off.uc().x,-self.metal1_pitch)])
        self.add_contact(self.poly_stack,(self.rack_rst_b_in_off.lx()-self.via_co_shift, -self.metal1_pitch-self.via_shift("co")))
        self.add_via(self.metal1_stack, (self.rack_rst_b_in_off.lx(), -self.metal1_pitch))
        
        pos1= (self.rack_rst_b_in_off.uc().x,-self.metal1_pitch+0.5*self.metal2_width)
        pos2= (self.rack_inst.lx()-2*self.metal1_pitch,pos1[1])
        pos3= (pos2[0], self.reset_bar_off_y)
        self.add_wire(self.metal1_rev_stack, [pos1, pos2, pos3])
        self.add_via(self.metal1_stack, (pos2[0]-0.5*self.metal1_width, self.reset_bar_off_y - self.via_shift("v1")))
        ...
        self.add_via(self.metal1_stack, (self.wack_rst_b_in_off.lx()+self.co_xshift, 2*contact.poly.height))
        
        pos1= (self.wack_rst_b_in_off.uc().x,2*contact.poly.height+0.5*self.metal2_width)
        pos2= (self.wack_inst.rx()+self.metal1_pitch,pos1[1])
        pos3= (pos2[0], self.reset_bar_off_y)
        self.add_wire(self.metal1_rev_stack,[pos1, pos2, pos3])
        self.add_via(self.metal1_stack, (pos2[0]-0.5*self.metal1_width, self.reset_bar_off_y - self.via_shift("v1")))
    ...
    def add_input_wreq_pin(self):
        ...
        self.add_path("metal1",[(self.wack_wreq_in_off.lx()+0.5*self.metal1_width+2*self.co_xshift, contact.poly.height), 
                                (self.wack_wreq_in_off.lx()+0.5*self.metal1_width+2*self.co_xshift, self.min_off_y)])
    ...
    def add_input_rreq_pin(self):
        ...
        self.add_path("metal1",[(self.rack_rreq_in_off.lx()+2*self.co_xshift+0.5*self.metal1_width, 0), 
                                (self.rack_rreq_in_off.lx()+2*self.co_xshift+0.5*self.metal1_width, self.min_off_y)])

        self.add_rect(layer="metal2",
                      offset= (self.rack_rreq_in_off.uc().x, self.rreq_off_y),
                      width= self.pin_width-self.rack_rreq_in_off.uc().x-self.metal1_pitch,
                      height=self.metal2_width)
        self.add_via(self.metal1_stack, (self.rack_rreq_in_off.uc().x,self.rreq_off_y - self.via_shift("v1")))
    ...
    def add_input_dr_pins(self):
        ...
        if self.num_subanks > 1:
            for i in range(self.num_subanks):
                self.add_path("poly",[self.DR_off[i].uc(), 
                                     (self.DR_off[i].uc().x, -2*i*self.metal1_pitch)])
                self.add_contact(self.poly_stack,
                                 (self.DR_off[i].lx(),-2*i*self.metal1_pitch-self.via_shift("co")))

                x_off=self.DR_off[i].lx()+0.5*contact.poly.width
                self.add_path("metal1",[(x_off, -2*i*self.metal1_pitch),
                                        (x_off, self.data_ready_off_y- i*self.metal2_pitch)])
                if ((-self.data_ready_off_y- i*self.metal2_pitch) < (self.minarea_metal1/contact.m1m2.width)):
                    self.add_m1_minarea((self.DR_off[i].lx()+self.co_xshift+0.5*contact.m1m2.width, self.data_ready_off_y-(i-1)*self.metal2_pitch))
                self.add_via(self.metal1_stack, (self.DR_off[i].lx()+self.co_xshift,
                                             self.data_ready_off_y- i*self.metal2_pitch-self.via_shift("v1")))

                self.add_rect(layer="metal2", 
                              offset=(-self.metal1_pitch,
                                      self.data_ready_off_y- i*self.metal2_pitch),
                              width=self.width,
                ...
                self.add_rect(layer="metal2", 
                              offset=(-self.metal1_pitch,self.dr_off_y), 
        ...
        self.add_path("poly",[self.rack_dr_in_off.uc(), 
                             (self.rack_dr_in_off.uc().x, self.rack_inst.uy()+self.metal1_pitch)])
        self.add_contact(self.poly_stack,(self.rack_dr_in_off.lx()-self.via_co_shift, 
                                          self.rack_inst.uy()+self.metal1_pitch-contact.poly.height+self.via_shift("co")))
        self.add_via(self.metal1_stack, (self.rack_dr_in_off.lx(), 
                                      self.rack_inst.uy()+self.metal1_pitch-contact.m1m2.height))

        pos1= (self.rack_dr_in_off.uc().x, self.rack_inst.uy()+self.metal1_pitch-0.5*self.metal2_width)
        pos2= (self.rack_inst.lx()-self.metal1_pitch,pos1[1])
        pos3= (pos2[0],self.dr_off_y)
        self.add_wire(self.metal1_rev_stack,[pos1, pos2, pos3])
        self.add_via(self.metal1_stack, (pos2[0], self.dr_off_y - self.via_shift("v1")))
        
        # Add data_ready-gate output
        if self.num_subanks > 1:
            x_off = self.dr_inst.rx()+2*self.metal1_pitch
            self.add_via(self.metal1_stack, (x_off, self.dr_off_y - self.via_shift("v1")))
            self.add_wire(self.metal1_rev_stack,[self.dr_off.uc(),
                           (self.dr_off.uc().x, self.comp_off_y+0.5*self.metal2_width ), 
                           (x_off, self.comp_off_y+0.5*self.metal2_width),
    ...
    def add_input_wc_pins(self):
        ...
        if self.num_subanks > 1:
            for i in range(self.num_subanks):
                self.add_path("poly",[self.WC_off[i].uc(),(self.WC_off[i].uc().x,-2*i*self.metal1_pitch)])
                self.add_contact(self.poly_stack, (self.WC_off[i].lx(),-2*i*self.metal1_pitch-self.via_shift("co")))

                x_off = self.WC_off[i].lx()+0.5*contact.poly.width
                self.add_path("metal1",[(x_off,-2*i*self.metal1_pitch), 
                                        (x_off,self.write_comp_off_y- i*self.metal2_pitch)])
                self.add_via(self.metal1_stack, (self.WC_off[i].lx()+self.co_xshift,
                                             self.write_comp_off_y- i*self.metal2_pitch-self.via_shift("v1")))

                self.add_rect(layer="metal2", 
                              offset=(-self.metal1_pitch,
                                       self.write_comp_off_y-i*self.metal2_pitch),
                              width=self.width,
                              height = contact.m2m3.first_layer_height)
                ...
                self.add_rect(layer="metal2", 
                                    offset=(-self.metal1_pitch,self.wc_off_y), 
                                    width=self.dec_en_inst.rx(),
                                    height = contact.m1m2.width)
        ...
        self.add_via(self.metal1_stack, (self.wack_wc_in_off.lx(),
                                     self.wack_inst.uy()-contact.m1m2.height))

        pos1= (self.wack_wc_in_off.uc().x, self.wack_inst.uy()-0.5*self.metal2_width)
        pos2= (self.wack_inst.rx()+2*self.metal1_pitch, pos1[1])
        pos3= (pos2[0], self.wc_off_y)
        self.add_wire(self.metal1_rev_stack, [pos1, pos2, pos3])
        self.add_via(self.metal1_stack, (pos2[0]-0.5*self.metal1_width, self.wc_off_y - self.via_shift("v1")))
        
        # Add write_complete-gate output
        if self.num_subanks > 1:
            x_off =  self.wc_inst.lx()-self.metal1_pitch
            self.add_via(self.metal1_stack, (x_off, self.wc_off_y - self.via_shift("v1")))
            self.add_wire(self.metal1_rev_stack,
                          [self.wc_off.uc(), (self.wc_off.uc().x, self.comp_off_y+0.5*self.metal2_width), 
                          (x_off, self.comp_off_y+0.5*self.metal2_width),
    ...
    def add_input_go_pin(self):
        ...
        for i in range(self.num_subanks):
             self.add_rect(layer="metal2", 
                           offset=(-self.metal1_pitch,self.go_off_y-i*self.metal1_pitch), 
                           width=self.width,
                           height=contact.m1m2.width)
             ...
             self.add_path("poly",[self.wc_go_off[i].uc(),(self.wc_go_off[i].uc().x, -(2*i+1)*self.metal1_pitch)])
             self.add_contact(self.poly_stack, (self.wc_go_off[i].lx(),-(2*i+1)*self.metal1_pitch-self.via_shift("co")))

             x_off = self.wc_go_off[i].lx()+0.5*contact.poly.width
             self.add_path("metal1",[(x_off,-(2*i+1)*self.metal1_pitch),(x_off, self.go_off_y-i*self.metal1_pitch)])
              
             self.add_via(self.metal1_stack, (self.wc_go_off[i].lx()+ self.co_xshift,
                                          self.go_off_y-i*self.metal1_pitch-self.via_shift("v1")))

             self.add_path("poly",[self.dr_go_off[i].uc(),(self.dr_go_off[i].uc().x, -(2*i+1)*self.metal1_pitch)])
             self.add_contact(self.poly_stack,(self.dr_go_off[i].lx(),-(2*i+1)*self.metal1_pitch-self.via_shift("co")))

             x_off = self.dr_go_off[i].lx()+0.5*contact.poly.width
             self.add_path("metal1",[(x_off,-(2*i+1)*self.metal1_pitch),(x_off, self.go_off_y-i*self.metal1_pitch)])
              
             self.add_via(self.metal1_stack, (self.dr_go_off[i].lx()+ self.co_xshift,
                                          self.go_off_y-i*self.metal1_pitch-self.via_shift("v1")))
    ...
    def add_output_ack_pin(self):
        ...
        x_off=self.ack_inst.lx()-4*self.metal1_pitch
        if self.power_gate:
            x_off = self.pg_nor[0].lx() - self.metal1_pitch
            pos1 = self.pg_nor[0].get_pin("Z").uc()
            pos2 = vector(pos1.x, pos1.y+self.metal1_pitch)
            pos3 = vector(x_off, pos2.y)
            pos4 = vector(x_off,self.min_off_y)
            self.add_wire(self.metal1_rev_stack, [pos1, pos2, pos3, pos4])
        
            x1_off=self.ack_inst.lx()-4*self.metal1_pitch
            ...
            self.add_wire(self.metal1_rev_stack, [pos1, pos2, pos3, pos4, pos5])
        
        else:
            self.add_wire(self.metal1_rev_stack, [self.ack_out_off, 
                                             (x_off, self.ack_out_off.y), (x_off, self.min_off_y)])

        self.add_layout_pin(text="ack", 
                            layer="metal1", 
                            offset=(x_off-0.5*self.metal1_width,self.min_off_y), 
                            width=self.metal1_width)
        
        self.add_via(self.metal1_stack, (x_off-0.5*self.metal1_width, self.ack_off_y - self.via_shift("v1")))
        self.add_rect(layer="metal2", 
        ...
        self.add_path("poly",[self.u_ack_in_off.uc(),(self.u_ack_in_off.uc().x,-self.metal1_pitch)])
        self.add_contact(self.poly_stack, (self.u_ack_in_off.lx(),-self.metal1_pitch-self.via_shift("co")))
        x_off = self.u_ack_in_off.lx()+0.5*contact.poly.width
        self.add_path("metal1", [(x_off,-self.metal1_pitch), (x_off, self.ack_off_y)])
        self.add_via(self.metal1_stack, (self.u_ack_in_off.lx(), self.ack_off_y - self.via_shift("v1")))
    ...
    def add_output_wen_pin(self):
        ...
        self.add_rect(layer="metal2", 
                      offset=(-self.metal1_pitch,self.wen_off_y), 
                      width=self.width,
                      height=contact.m1m2.width)
        ...
        self.add_via(self.metal1_stack, (self.wen_out_off.rx()+ self.via_shift("v1"),self.wen_off_y), rotate=90)
    ...
    def add_output_wack_pin(self):
        ...
        x_off = self.wack_inst.rx()+3*self.metal1_pitch

        if self.power_gate:
            x_off = self.pg_nor[2].rx() + 4*self.metal1_pitch
            ...
            x1_off=self.wack_inst.rx()+3*self.metal1_pitch
            pos1 = self.wack_out_off
            pos2 = vector(x1_off, self.wack_out_off.y)
            pos3 = vector(x1_off, self.w_off_y+0.5*self.metal2_width)
            pos5= self.pg_inv[1].get_pin("A").uc()
            pos4 = vector(pos5.x, pos3.y)
            self.add_wire(self.metal1_rev_stack, [pos1, pos2, pos3, pos4, pos5])
        
        else:
            self.add_wire(self.metal1_rev_stack, [self.wack_out_off, 
        ...
        self.add_via(self.metal1_stack, (x_off-0.5*contact.m1m2.width,self.wack_off_y - self.via_shift("v1")))
        ...
        self.add_via(self.metal1_stack, (self.ack_wack_in_off1.rx()-contact.poly.width, self.wack_off_y - self.via_shift("v1")))
        ...
        self.add_via(self.metal1_stack, (self.ack_wack_in_off2.lx(), 
                                     self.ack_inst.uy()-contact.m1m2.height))
        pos1= (self.ack_wack_in_off2.lx(), self.ack_inst.uy()-0.5*self.metal2_width)
        pos2= (self.ack_inst.rx()+self.metal1_pitch, pos1[1])
        pos3= (pos2[0], self.wack_off_y)
        self.add_wire(self.metal1_rev_stack, [pos1, pos2, pos3])
        self.add_via(self.metal1_stack, (pos2[0], self.wack_off_y - self.via_shift("v1")))

    def add_output_pchg_pin(self):
        """ Adds the output pchg pin """

        self.add_rect(layer="metal2",
                      offset=(-self.metal1_pitch,self.pchg_off_y), 
                      width=self.width,
                      height=contact.m1m2.width)
        ...
        self.add_via(self.metal1_stack, (self.u_pre_pchg_in_off.lx()+self.co_xshift, 0))
        
        pos1= (self.u_pre_pchg_in_off.rx(), 0.5*self.metal2_width)
        pos2= (self.u_inst.lx()-self.metal1_pitch, pos1[1])
        pos3= (pos2[0], pchg_nor3_off.y-contact.m1m2.width)
        pos4= (pchg_nor3_off.x+self.metal1_width-self.via_shift("v1"), pos3[1])
        self.add_wire(self.metal1_rev_stack, [pos1, pos2, pos3, pos4])
        self.add_via(self.metal1_stack,(pos4[0]+contact.m1m2.height,pos3[1]-contact.m1m2.width),rotate=90)
        
        #Route the pchg rail
        self.add_via(self.metal1_stack, (self.pchg_off.rx()+ self.via_shift("v1"), self.pchg_off_y), rotate=90)
        ...
        self.add_via(self.metal1_stack, (self.ack_pchg_in_off.rx()-contact.poly.width+0.5*contact.m1m2.width, 2*contact.poly.height-self.via_shift("co")), rotate=90)

        
        pos1= (self.ack_pchg_in_off.uc().x, 2*contact.poly.height+0.5*self.metal2_width-self.via_shift("co"))
        pos2= (self.ack_inst.lx()-2*self.metal1_pitch, pos1[1])
        pos3= (pos2[0], self.pchg_off_y)
        self.add_wire(self.metal1_rev_stack,[pos1, pos2, pos3])
        self.add_via(self.metal1_stack, (pos2[0]+0.5*self.metal1_width+ self.via_shift("v1"), self.pchg_off_y), rotate=90)
        
        # decoder_enable_gate pchg-input connection
        self.add_via(self.metal1_stack, (self.dec_en_in_off.rx()+ self.via_shift("v1"), self.pchg_off_y), rotate=90)
        ...
        if self.num_subanks > 1:
            ...
            self.add_via(self.metal1_stack, (self.WC_pchg_off.rx(), 
                                         self.wc_inst.uy()-2*contact.poly.height))

            self.add_wire(self.metal1_rev_stack, [(self.WC_pchg_off.rx(),
                          self.wc_inst.uy()-contact.poly.height-0.5*self.metal2_width), 
                          (self.wc_inst.lx()-2*self.metal1_pitch,
                          self.wc_inst.uy()-contact.poly.height-0.5*self.metal2_width),
                          (self.wc_inst.lx()-2*self.metal1_pitch,self.pchg_off_y)])
            self.add_via(self.metal1_stack, (self.wc_inst.lx()-2*self.metal1_pitch+0.5*self.metal1_width+ self.via_shift("v1"),
            ...
            self.add_via(self.metal1_stack, off)

            pos1= (self.DR_pchg_off.rx(), self.dr_inst.uy()-contact.poly.height-0.5*self.metal2_width)
            pos2= (self.dr_inst.rx()+self.metal1_pitch, pos1[1])
            pos3= (pos2[0], self.pchg_off_y)
            self.add_wire(self.metal1_rev_stack, [pos1, pos2, pos3])
            self.add_via(self.metal1_stack, (pos2[0]+0.5*self.metal1_width +self.via_shift("v1"),self.pchg_off_y), rotate=90)
    ...
    def add_output_rack_pin(self):
        ...
        x_off = self.rack_inst.rx()+2*self.metal1_pitch
        if self.power_gate:
            x_off = self.pg_nor[2].rx() + self.metal1_pitch 
            pos1 = self.pg_nor[2].get_pin("Z").uc()
            pos2 = vector(pos1.x, pos1.y+2*self.metal1_pitch)
            pos3 = vector(x_off, pos2.y)
            pos4 = vector(x_off,self.min_off_y)
            self.add_wire(self.metal1_rev_stack, [pos1, pos2, pos3, pos4])
        
            x1_off=self.rack_inst.rx()+2*self.metal1_pitch
            pos1 = self.rack_out_off
            pos2 = vector(x1_off, self.rack_out_off.y)
            pos3 = vector(x1_off, self.dec_en_off_y-self.metal2_pitch+0.5*self.metal2_width)
            pos5= self.pg_inv[2].get_pin("A").uc()
            pos4 = vector(pos5.x, pos3.y)
            self.add_wire(self.metal1_rev_stack, [pos1, pos2, pos3, pos4, pos5])
        
        else:
            self.add_wire(self.metal1_rev_stack, [self.rack_out_off, (x_off, self.rack_out_off.y), (x_off, self.min_off_y)])

        self.add_layout_pin(text="rack", 
                            layer="metal1", 
                            offset=(x_off-0.5*contact.m1m2.width, self.min_off_y), 
                            width=self.metal1_width)
        self.add_via(self.metal1_stack, (x_off-0.5*self.metal1_width,self.rack_off_y - self.via_shift("v1")))
        ...
        self.add_contact(self.poly_stack, (self.wen_rack_in_off.rx()+self.poly_to_active-contact.poly.width,
                                           -self.metal1_pitch-self.via_shift("co")))
        ...
        self.add_via(self.metal1_stack,(x_off-0.5*contact.poly.width, self.rack_off_y - self.via_shift("v1")))

        # ack-gate rack-input connection
        self.add_path("poly",[self.ack_rack_in_off.uc(), (self.ack_rack_in_off.uc().x, -self.metal1_pitch)])
        self.add_contact(self.poly_stack,(self.ack_rack_in_off.rx()-contact.poly.width, -self.metal1_pitch-self.via_shift("co")))
        self.add_path("metal1",[(self.ack_rack_in_off.rx()-contact.poly.width+0.5*contact.poly.width, -self.metal1_pitch), 
                                (self.ack_rack_in_off.rx()-contact.poly.width+0.5*contact.poly.width, self.rack_off_y)])
        self.add_via(self.metal1_stack, (self.ack_rack_in_off.rx()-contact.poly.width,self.rack_off_y - self.via_shift("v1")))
    ...
    def add_output_dec_en_pin(self):
        ...
        self.add_rect(layer="metal2", 
                      ...
                      width=self.width-self.dec_en_out_off.uc().x-self.metal1_pitch,
                      height=contact.m1m2.width)
        ...
        self.add_via(self.metal1_stack, (self.dec_en_out_off.lx(),self.dec_en_off_y - self.via_shift("v1")))
    
    def add_U_routing(self):
        """ Routes the u signal """

        self.add_rect(layer="metal2", 
                      offset=(-self.metal1_pitch, self.U_off_y), 
                      width=self.pin_width, 
                      height = contact.m1m2.width)
        self.add_via(self.metal1_stack, (self.u_out_off.uc().x-0.5*contact.m1m2.width,self.U_off_y - self.via_shift("v1")))
        ...
        self.add_contact(self.poly_stack, (self.wen_u_in_off.lx()+self.poly_to_active-self.via_co_shift+self.co_xshift, 
                                           -self.metal1_pitch-self.via_shift("co")))
        self.add_via(self.metal1_stack, (self.wen_u_in_off.lx()+self.poly_to_active+self.co_xshift, -self.metal1_pitch))
        
        pos1= (self.wen_u_in_off.uc().x+self.poly_to_active, -self.metal1_pitch+0.5*self.metal2_width)
        pos2= (self.wen_inst.lx()-self.metal1_pitch,pos1[1])
        pos3= (pos2[0], self.U_off_y)
        self.add_wire(self.metal1_rev_stack, [pos1, pos2, pos3])
        self.add_via(self.metal1_stack, (pos2[0]-0.5*self.metal2_width, self.U_off_y - self.via_shift("v1")))
        ...
        self.add_contact(self.poly_stack, (x_off-self.via_co_shift, -self.metal1_pitch-self.via_shift("co")))
        self.add_via(self.metal1_stack, (x_off, -self.metal1_pitch))
        
        pos1= (self.sen_u_in_off.lx()-self.poly_to_active, -self.metal1_pitch+0.5*self.metal2_width)
        pos2= (self.sen_inst.rx()+self.metal1_pitch, pos1[1])
        pos3= (pos2[0], self.U_off_y)
        self.add_wire(self.metal1_rev_stack, [pos1, pos2, pos3])
        self.add_via(self.metal1_stack, (pos2[0]-0.5*self.metal1_width,self.U_off_y - self.via_shift("v1")))
        ...
        self.add_via(self.metal1_stack, (self.rack_u_in_off.lx()+self.co_xshift, 2*contact.poly.height))

        pos1=(self.rack_u_in_off.uc().x, 2*contact.poly.height+0.5*self.metal2_width)
        pos2=(self.rack_inst.rx()+self.metal1_pitch, pos1[1])
        pos3=(pos2[0], self.U_off_y)
        self.add_wire(self.metal1_rev_stack, [pos1, pos2, pos3])
        self.add_via(self.metal1_stack, (pos2[0]-0.5*self.metal1_width, self.U_off_y - self.via_shift("v1")))
        ...
        self.add_path("poly",[self.wack_u_in_off.uc(),
                             (self.wack_u_in_off.uc().x,-self.metal1_pitch)])
        self.add_contact(self.poly_stack,(self.wack_u_in_off.lx()-self.via_co_shift, -self.metal1_pitch-self.via_shift("co")))
        self.add_via(self.metal1_stack, (self.wack_u_in_off.lx(), -self.metal1_pitch))

        pos1= (self.wack_u_in_off.uc().x,-self.metal1_pitch+0.5*self.metal2_width)
        pos2= (self.wack_inst.lx()-self.metal1_pitch,pos1[1])
        pos3= (pos2[0], self.U_off_y)
        self.add_wire(self.metal1_rev_stack, [pos1, pos2, pos3])
        self.add_via(self.metal1_stack, (pos2[0]-0.5*self.metal1_width, self.U_off_y - self.via_shift("v1")))
    ...
    def add_output_sen_pin(self):
        ...
        self.add_rect(layer="metal2", 
                      offset=(-self.metal1_pitch,self.sen_off_y), 
                      width=self.width,
                      height=contact.m1m2.width)
        ...
        pos2= vector(pos1.x, pos1.y+2*self.metal1_pitch)
        pos3= vector(self.pin_width+2*self.metal1_pitch, pos2.y)
        pos4=vector(pos3.x, self.sen_off_y)
        self.add_wire(self.metal1_rev_stack, [pos1, pos2, pos3, pos4])
        self.add_via(self.metal1_stack, pos4, rotate=90)

    def add_dc_routing(self):
        """ Routes the dc input, output and power signals """
        
        pos1= self.sen_out_off.uc()
        pos2= vector(pos1.x, self.dc_inst.by()-self.metal1_pitch)
        pos3= vector(self.xsen_in_off.x, pos2.y)
        pos4 = self.xsen_in_off
        if abs(pos1.x-pos3.x)<self.metal1_pitch:
    ...
    def route_vdd_gnd(self):
        ...
        self.add_rect(layer="metal2",
                      offset=(-self.metal1_pitch, self.vdd_off_y), 
                      width=self.width,
                      height=size)
        ...
        self.add_rect(layer="metal2", 
                      offset=(-self.metal1_pitch, self.gnd_off_y), 
                      width=self.width,
                      height=size)
        ...
        for i in range(2):
            pos1= self.dc_inst.get_pin(pins[i]).lc()-vector(0, (i+2)*self.metal1_pitch+0.5*contact.m1m2.width)
            pos2=vector(self.pin_width-(i+1)*self.metal1_pitch, pos1.y)
            pos3=vector(pos2.x, off[i])
            self.add_wire(self.metal1_rev_stack,[pos1, pos2, pos3])
            self.add_via(self.metal1_stack,(pos3.x-0.5*contact.m1m2.width, off[i]-self.via_shift("v1")))
        ...
        for inst in gates_list:
            for gnd_pin in inst.get_pins("gnd"):
                ...
                self.add_via(self.metal1_stack, (gnd_pin.uc().x-0.5*contact.m1m2.width, self.gnd_off_y-self.via_shift("v1")))
            ...
            for vdd_pin in inst.get_pins("vdd"):
                ...
                self.add_via(self.metal1_stack, (vdd_pin.uc().x-0.5*contact.m1m2.width, self.vdd_off_y-self.via_shift("v1")))

        if info["has_nimplant"]:
            if self.num_subanks > 1:
                off = (2*self.num_subanks-1)*self.metal1_pitch
            else:
                off = 3*self.metal1_pitch
