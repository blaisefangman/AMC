...
class multi_bank(design.design):
    ...
    def compute_bus_sizes(self):
        ...
        self.bnk_to_bus_gap= 2*self.metal3_width
        self.bnk_to_bnk_gap= 2*self.metal3_width
    ...
    def add_two_banks(self):
        ...
        sp_mrg_ctrl_off= vector(self.v_bus_off.x+self.v_bus_width -contact.m1m2.height,
                                self.v_bus_off.y+ self.v_bus_height-self.metal1_width)
    ...
    def add_four_banks(self):
        ...
        if self.orien == "V":
            ...
            if self.mask:
                self.width += self.w_size*self.pitch
        sp_mrg_ctrl_off= vector(self.v_bus_off.x+self.v_bus_width-self.pitch+1.5*self.metal2_width,
                                self.v_bus_off.y+ self.v_bus_height-self.metal1_width)
        self.sp_mrg_ctrl_inst= self.add_inst(name="split_merge_control", 
                                             mod=self.sp_mrg_ctrl, 
                                             offset=sp_mrg_ctrl_off,
    ...
    def add_single_bank_pins(self):
        ...
        if self.mask:
            ...
            stack_list = [self.metal1_stack, self.metal2_stack]
        else:
            pin_list = ["din"]
            layer_list = ["metal1"]
            stack_list = [self.metal1_stack]
        ...
        if self.num_subanks>1:
            ...
            for i in range(self.w_size):
                ...
                for j in range(self.num_subanks):
                    ...
                    self.add_via(self.metal1_stack,(offset.x, offset.y-self.via_shift("v1")))
        ...
        for i in range(2):
            ...
            for j in range(2*self.num_subanks+1):
                ...
                self.add_via_center(self.metal2_stack, (offset1.x, yoff+0.5*self.pow_width-self.metal1_width)+self.pow_via_shift, size = [self.num_via,self.num_via])
                self.add_rect(layer="metal2",
    ...
    def route_banks(self):
                ...
                for i in range(self.num_subanks):
                  for j in range(self.w_size):
                      din_off= vector(self.bank_inst[k].get_pin("din[{0}][{1}]".format(i,j)).lx(), 
                                      self.din1_bus_off.y+ j*self.pitch + 0.5*self.metal1_width)
                      ...
                      self.add_rect(layer="metal2", 
                                    ...
                                    width=self.metal2_width, 
                                    height=din_height)
                      self.add_via(self.metal1_stack, (din_off.x, din_off.y-self.via_shift("v1")))
                      ...
                      if self.mask:
                          ...
                          self.add_rect(layer="metal2", 
                                        ...
                                        width=self.metal2_width, 
                                        height=din_height)
                          self.add_via(self.metal2_stack, (bm_off.x, bm_off.y-self.via_shift("v1")))
    
                      
                      dout_off= vector(self.bank_inst[k].get_pin("dout[{0}][{1}]".format(i,j)).lx(), 
                                       self.dout1_bus_off.y+ j*self.pitch + 0.5*self.metal1_width)
                      ...
                      self.add_rect(layer="metal2", 
                                    ...
                                    width=self.metal2_width, 
                                    height=dout_height)
                      self.add_via(self.metal1_stack, (dout_off.x, dout_off.y-self.via_shift("v1")))
        ...
        if (self.num_banks == 4 and  self.orien == "V"):
            for k in range(self.num_banks):
                for i in range(self.num_subanks):
                  for j in range(self.w_size):
                      ...
                      din_off= vector(self.bank_inst[k].get_pin("din[{0}][{1}]".format(i,j)).lx(), 
                                      self.data_in_bus_off[k//2]+ j*self.pitch + 0.5*self.metal1_width)
                      din_height=  self.bank_inst[k].get_pin("din[{0}][{1}]".format(i,j)).by() - \
                      ...
                      dout_off= vector(self.bank_inst[k].get_pin("dout[{0}][{1}]".format(i,j)).lx(), 
                                self.data_out_bus_off[k//2]+ j*self.pitch + 0.5*self.metal1_width)
                      dout_height=  self.bank_inst[k].get_pin("dout[{0}][{1}]".format(i,j)).by() -\
            ...
            for j in range(self.w_size):                      
                ...
                doutoff1= vector(self.dout1_bus_off.x, self.dout1_bus_off.y + j*self.pitch + self.metal1_width)
                doutoff2= vector(-(j+2)*self.pitch, doutoff1.y)
                doutoff3= vector(doutoff2.x, self.dout2_bus_off.y + j*self.pitch + self.metal1_width)
                doutoff4= vector(self.dout2_bus_off.x, doutoff3.y)
                self.add_wire(self.metal1_stack, [doutoff1, doutoff2, doutoff3, doutoff4])
                

                dinoff1= vector(self.din1_bus_off.x+self.data_bus_width, self.din1_bus_off.y + j*self.pitch + self.metal1_width)
                dinoff2= vector(dinoff1.x+(j+1)*self.pitch, dinoff1.y)
                dinoff3= vector(dinoff2.x,  self.din2_bus_off.y + j*self.pitch + self.metal1_width)
                dinoff4= vector(self.din2_bus_off.x, dinoff3.y)
                self.add_wire(self.metal1_stack, [dinoff1, dinoff2, dinoff3, dinoff4])
                      
                if self.mask:
                    bmoff1= vector(self.din1_bus_off.x+self.data_bus_width, self.din1_bus_off.y + j*self.pitch + self.metal3_width)
                    bmoff2= vector(bmoff1.x+(j+1+self.w_size)*self.pitch, bmoff1.y)
                    bmoff3= vector(bmoff2.x,  self.din2_bus_off.y + j*self.pitch + self.metal3_width)
                    bmoff4= vector(self.din2_bus_off.x, bmoff3.y)
                    self.add_wire(self.metal2_rev_stack, [bmoff1, bmoff2, bmoff3, bmoff4])
        ...
        for k in range(self.num_banks):
            for i in range(self.bank_addr_size):
                ...
                addr_off= vector(self.addr_bus1_off.x+ i*self.pitch- self.metal3_enclose_via2, 
        ...
        if not self.two_level_bank:
            sel_pos1=vector(self.v_ctrl_bus_pos["S"].cx(), self.v_ctrl_bus_pos["S"].by()+self.metal1_width)
            sel_pos2=vector(self.v_ctrl_bus_pos["vdd"].cx(), self.v_ctrl_bus_pos["vdd"].by()+self.metal1_width)
            self.add_path("metal1", [sel_pos1, sel_pos2])
            self.add_via(self.metal1_stack, (sel_pos1.x-0.5*self.metal2_width, 
                         sel_pos1.y-0.5*self.metal1_width-self.via_shift("v1")))
            self.add_via(self.metal1_stack, (sel_pos2.x-0.5*self.metal2_width, 
                         sel_pos2.y-0.5*self.metal1_width-self.via_shift("v1")))
        ...
        if (self.num_banks == 2 or self.orien == "H"):
            for k in range(self.num_banks):
                for vdd_pin in self.bank_inst[k].get_pins("vdd"):
                    vdd_off= vector(vdd_pin.lx(), self.pow_rail_1_off.y + 0.5*self.metal1_width)
                    vdd_height=  vdd_pin.by() - self.pow_rail_1_off.y
                    self.add_rect(layer="metal2", 
                                  offset=vdd_off, 
                                  width=self.pow_width, 
                                  height=vdd_height)
                    self.add_via_center(self.metal2_stack, (vdd_off.x, vdd_off.y)+self.pow_via_shift, size=[self.num_via, self.num_via])
        
                for gnd_pin in self.bank_inst[k].get_pins("gnd"):
                    gnd_off= vector(gnd_pin.lx(), 
                                    self.pow_rail_1_off.y + self.pow_pitch + 0.5*self.metal1_width)
                    gnd_height=  gnd_pin.by() - self.pow_rail_1_off.y - self.pow_pitch
                    self.add_rect(layer="metal2", 
                                  offset=gnd_off, 
                                  width=self.pow_width, 
                                  height=gnd_height)
                    self.add_via_center(self.metal2_stack, (gnd_off.x, gnd_off.y)+self.pow_via_shift, size=[self.num_via, self.num_via])

        if (self.num_banks == 4 and  self.orien == "V"):
            self.power_rail_off= [self.pow_rail_1_off.y, self.pow_rail_2_off.y]
            for k in range(self.num_banks):
                for vdd_pin in self.bank_inst[k].get_pins("vdd"):
                    vdd_off= vector(vdd_pin.lx(), self.power_rail_off[k//2] + 0.5*self.metal1_width)
                ...
                for gnd_pin in self.bank_inst[k].get_pins("gnd"):
                    gnd_off= vector(gnd_pin.lx(), self.power_rail_off[k//2] + \
                                    self.pow_pitch + 0.5*self.metal1_width)
        ...
        self.add_via_center(self.metal2_stack, (self.vdd_off.x+0.5*self.metal2_width, self.pow_rail_1_off.y+0.5*self.pow_width), size=[1, self.num_via])
        self.add_via_center(self.metal2_stack, (self.gnd_off.x+0.5*self.metal2_width, self.pow_rail_1_off.y+self.pow_pitch+0.5*self.pow_width), size=[1, self.num_via])

        if (self.num_banks == 4 and  self.orien == "V"):
            self.add_via(self.metal2_stack, (self.vdd_off.x-self.via_shift("v2"), self.pow_rail_2_off.y+\
                        0.5*self.metal1_width-self.via_shift("v1")), size=[1, self.num_via])
...
#/////////////////////////////////////////////////////////////////////////////#
    ...
    def add_split_merge_cells(self):
        ...
        for i in range(self.w_size):
            if i%2:
                ...
                off=self.dsplit_ary_inst.get_pin("D[{0}]".format(i)).lr()-vector(self.metal2_width,0)
            ...
            if self.mask:
                if i%2:
                    ...
                    off=self.dsplit_ary_inst.get_pin("bm_in[{0}]".format(i)).lr()-vector(self.metal2_width,0)
            ...
            for i in range(self.w_size):
                self.add_layout_pin(text="dout[{0}]".format(i), 
                                    ...
                                    offset= (self.dmerge_ary_inst.get_pin("Q[{0}]".format(i)).lx(),
                                             self.dmerge_ary_inst.uy()-self.metal2_width),
                                    width=self.metal2_width, 
                                    height=self.metal2_width)
    ...
    def route_data_split_merge(self):
        ...
        if (self.num_banks == 2 and self.orien == "H"):
            ...
            for i in range(self.w_size):
                ...
                self.add_wire(self.metal1_stack, 
                              ...
                              (self.dsplitoff[i].x, self.dsplitoff[i].y+0.5*self.metal1_width)]) 
                if self.mask:
                    shift= abs(self.metal3_width-self.metal1_width)
                    self.add_wire(self.metal2_rev_stack, 
                                  ...
                                  (self.dsplitoff[i].x, self.dsplitoff[i].y+0.5*self.metal3_width)]) 


        # connecting data_in_split_array to data_in_split_bus
        for i in range(self.w_size):
            din_pos= self.dsplit_ary_inst.get_pin("Q[{0}]".format(i)).uc()-vector(0, self.metal2_width)
            #x_off= self.bank_inst[self.num_banks-1].get_pin("din[{0}][{1}]".format(self.num_subanks-1,i)).lx()
            y_off= self.dsplitoff[i].y+0.5*self.metal2_width
            mid_pos=vector(self.bank_inst[1].get_pin("din[{0}][{1}]".format(self.num_subanks-1, i)).uc().x, y_off)
            #self.add_via_center(self.metal1_stack, (din_pos.x, y_off))
            self.add_path("metal2", [din_pos, mid_pos, (din_pos.x, y_off)])
            if (self.num_banks==4 or self.orien == "V"):
                self.add_via_center(self.metal1_stack, (din_pos.x, y_off))

            if self.mask:
                bm_pos= self.dsplit_ary_inst.get_pin("bm_out[{0}]".format(i)).uc()-vector(0, self.metal2_width)
                y_off= self.dsplitoff[i].y+0.5*self.metal3_width
                ...
                #self.add_via_center(self.metal2_stack, (bm_pos.x, y_off))
            ...
            if self.num_banks == 4:
                for i in range(self.w_size):
                    ...
                    self.add_wire(self.metal1_stack, 
                                  ...
                                  (self.dmrgoff[i].x, 
                                   self.dmrgoff[i].y+0.5*self.metal1_width)]) 
        ...
        if self.orien == "V":
            ...
            for i in range(self.w_size):
                ...
                self.add_wire(self.metal1_stack, [(data_out_pos_x, data_out_pos_y),
                             (data_out_pos_x+(i+1)*self.pitch, data_out_pos_y),
                             (data_out_pos_x+(i+1)*self.pitch, self.dmrgoff[i].y),
                             (self.dmrgoff[i].x,self.dmrgoff[i].y+0.5*self.metal1_width)]) 
        ...
        for i in range(self.w_size):
            ...
            if self.orien=="V":
                ...
                pos2=vector(data_out_pos.x, data_out_pos.y+self.metal2_width)
            if self.orien=="H":
            ...
            if (self.num_banks == 4 or self.orien=="V"):
                self.add_via(self.metal1_stack, (pos1.x+0.5*contact.m1m2.height, y_off), rotate=90)
    ...
    def route_addr_ctrl_split_merge(self):
        ...
        for i in range(self.addr_size):
            ...
            self.add_via(self.metal2_stack, (addr_split_pos[0]-0.5*self.metal3_width,
                                         addr_split_pos[1]-0.5*self.metal3_width))
            self.add_via(self.metal2_stack,(addr_bus_pos[0]-0.5*self.metal3_width,
                                        addr_split_y_off-0.5*self.metal3_width-self.via_shift("v2")))
        ...
        for i in range(self.control_size):
            ...
            self.add_wire(self.metal1_stack, [ctrl_pos[i]-vector(0, self.metal2_width), (ctrl_pos[i].x,ctrl_split_y_off),
    ...
    def route_split_cells_powers_and_selects(self):
        ...
        if self.power_gate:
            ...
            pos2 = vector(self.v_bus_off.x-10*self.pitch+0.5*self.metal2_width, pos1.y)
            pos3 = vector(pos2.x, offset.y+0.5*self.metal1_width)
            ...
            #self.add_via_center(self.metal1_stack, pos3+vector(0, 0.5*contact.m1m2.width), rotate=90)
        ...
        for i in range(3):
            for mod in (mod_list1 + mod_list2):
                ...
                self.add_wire(self.metal1_stack,[power_pos.lc(), 
                              (power_pos.lx()-(1+i)*self.pitch, power_pos.lc().y), 
                              (power_pos.lx()-(1+i)*self.pitch, y_off-(i+1)*self.pitch)])
                self.add_via(self.metal1_stack,(power_pos.lx()-(1+i)*self.pitch-\
                                            0.5*self.metal2_width,y_off-(i+1)*self.pitch)) 

        for mod in mod_list1:
            select_pos= mod.get_pin("S")
            self.add_wire(self.metal1_stack, [select_pos.lc(),
                          (select_pos.lx()-4*self.pitch, select_pos.lc().y), 
                          (select_pos.lx()-4*self.pitch, y_off-4*self.pitch)])
            self.add_via(self.metal1_stack, (select_pos.lx()-4*self.pitch-0.5*self.metal2_width,
                                         y_off-4*self.pitch)) 

        for mod in mod_list2:
            select_pos= mod.get_pin("M")
            self.add_wire(self.metal1_stack, [select_pos.lc(),(select_pos.lx()-4*self.pitch, select_pos.lc().y), 
                          (select_pos.lx()-4*self.pitch,y_off-4*self.pitch)])
            self.add_via(self.metal1_stack, (select_pos.lx()-4*self.pitch-0.5*self.metal2_width,
                                         y_off-4*self.pitch)) 
        ...
        for i in range(2):
            ...
            pos4=pos3-vector(0.5*self.metal2_width, 0)
        ...
        for mod in mod_list:           
            ...
            self.add_wire(self.metal1_stack, [pos1.lc(),pos2, pos3, pos4])
        ...
        self.add_wire(self.metal1_stack, [pos1, pos2, pos3, pos4, ctrl_bus_pos]) 
        ...
        for i in range(4):
            ...
            if self.orien == "H":
                ...
                self.add_via(self.metal1_stack, 
                            (select_pos.lx()+self.dmerge_ary.width+(1+i)*self.pitch-\
                            0.5*self.metal2_width,y_off-(1+i)*self.pitch)) 
            ...
            if self.orien == "V":
                ...
                self.add_wire(self.metal1_stack, [select_pos.lc(), 
                              ...
                              (select_pos.lx(), y_off-(1+i)*self.pitch+0.5*self.metal1_width)])
        ...
        for i in range(2):
            ...
            self.add_wire(self.metal1_stack, [(spl_mrg_power.lx(), spl_mrg_power.lc().y),
                         (spl_mrg_power.lx()-(i+8)*self.pitch, spl_mrg_power.lc().y),
                         (spl_mrg_power.lx(), self.reset_off.y-(2-i)*self.pitch),
                         (bank_power.uc().x+0.5*self.metal2_width, self.reset_off.y-(2-i)*self.pitch),
                         (bank_power.uc().x+0.5*self.metal2_width, self.reset_off.y+self.metal2_width)])
        ...
        sel_pos2=vector(self.reset_off.x-9*self.pitch-0.5*self.metal2_width, sel_pos1.y)
        sel_pos3=vector(sel_pos2.x, self.addr_split_ary_inst.by()-4*self.pitch)
        self.add_wire(self.metal2_rev_stack, [sel_pos0, sel_pos1, sel_pos2, sel_pos3])
        self.add_via_center(self.metal1_stack, sel_pos3)
        self.add_via_center(self.metal2_stack, sel_pos1, rotate=90)
        
        spl_mrg_reset= self.addr_split_ary_inst.get_pin("reset")
        self.add_wire(self.metal1_stack, [(spl_mrg_reset.lx(), spl_mrg_reset.lc().y),
                     (spl_mrg_reset.lx()-3*self.pitch, spl_mrg_reset.lc().y),
                     (spl_mrg_reset.lx()-3*self.pitch,self.reset_off.y-3*self.pitch),
                     (self.reset_off.x+0.5*self.metal2_width, self.reset_off.y-3*self.pitch),
                     (self.reset_off.x+0.5*self.metal2_width, self.reset_off.y+self.pitch)])

        spl_mrg_ack_merge= self.v_ctrl_bus_pos["ack_merge"].bc()
        self.add_wire(self.metal1_stack, [(spl_mrg_ack_merge.x, spl_mrg_ack_merge.y),
        ...
        self.add_layout_pin(text="ack_merge",
                            layer= "metal2",
                            offset=(self.reset_off.x-6*self.pitch-0.5*self.metal2_width, 
                                    self.ctrl_split_ary_inst.by()-8*self.pitch),
                            width= self.metal2_width,
                            height= self.metal2_width)
        ...
        if self.orien == "H":
            ...
            Mdout_off = (self.dmerge_ary_inst.rx()+6*self.pitch-0.5*self.metal2_width, yoff)
        if self.orien == "V":
            if self.mask:
            ...
            self.add_wire(self.metal1_stack,
                          ...
                          (x_off+index*self.pitch, 
                           en_data.lc().y), en_data.lc()])
            Mdout_off = (x_off+index*self.pitch-0.5*self.metal2_width, yoff)
