...
class starter_stopper(design.design):
    ...
    def create_layout(self):
        ...
        self.width= self.pmos1.rx()+self.m1_pitch
        self.height= self.inv_inst.uy()+self.m1_pitch+contact.m1m2.width
    ...
    def add_stopper(self):
        ...
        off = (max(self.inv.width,self.pull_up_pull_down.width)+ 5*self.m1_pitch, 0)
        ...
        off=(self.nand2_inst.rx()+self.nmos.height+ 2*max(self.gap, self.m1_pitch), 0)
        self.nmos1=self.add_inst(name="nmos1", mod=self.nmos, offset=off, rotate=90)
        self.connect_inst(["out", "_v", "gnd", "gnd"])

        off = (self.nmos1.rx()+self.pmos.height+ 2*max(self.gap, self.m1_pitch), 0)
    ...
    def starter_connections(self): 
        ...
        self.add_path("m1",[self.starter_inst.get_pin("Sp0").lc(), self.starter_inst.get_pin("Sn0").lc()])
    ...
    def connect_reset_to_sarter(self):
        ...
        pos4=vector(pos2.x-self.m2_pitch,pos3.y)
    ...
    def cross_couppled_nands_connections(self):    
        ...
        pos3=vector(pos2.x, mod1.by()-self.m1_pitch)
        pos4=vector(mod2.get_pin("B").lc().x-2*self.m1_pitch, pos3.y)
        ...
        pos2=vector(pos1.x+self.m2_width, pos1.y)
        pos3=vector(pos2.x, mod2.uy()+self.m1_pitch)
        pos4=vector(mod1.get_pin("B").lc().x-self.m1_pitch, pos3.y)
    ...
    def ptx_connections(self):    
        ...
        self.add_path("m1", [self.nmos1.get_pin("S").lc(),self.pmos1.get_pin("S").lc()])
        
        #connect gates and drain of nmos2 and pmos2
        self.add_path("poly", [self.nmos2.get_pin("G").lc(),self.pmos2.get_pin("G").lc()] )
        self.add_path("m1", [self.nmos2.get_pin("D").lc(),self.pmos2.get_pin("D").lc()])
        
        #connect gate of nmos1 to source of pmos2
        pos1=vector(self.nmos1.get_pin("G").lc().x+1.5*self.m1_pitch,self.nmos1.get_pin("G").uy())
        ...
        self.add_wire(self.m1_stack, [(pos1.x+0.5*self.m2_width, pos1.y), 
                                      (pos1.x, pos2),(pos3.x, pos2), pos3], widen_short_wires=False)
        self.add_via_center(self.m1_stack, self.pmos2.get_pin("S").cc(), rotate=90)
        
        width = max(ceil(self.minarea_m1/contact.poly.width),shift)
        ...
        pos1=vector(pin.lc().x+2.5*self.m1_pitch, pin.by()-contact.m1m2.height)
        ...
        self.add_path("m1", [self.nmos1.get_pin("D").uc(), self.nmos2.get_pin("S").uc()])
        pos1=self.nand1_inst.get_pin("gnd").lc()
        pos2=vector(self.nand1_inst.rx()+2*self.m1_pitch, pos1.y)
        pos3=vector(pos2.x, pos2.y-self.m1_pitch)
        pos5=self.nmos1.get_pin("D").uc()
        pos4=vector(pos5.x, pos3.y)
        
        self.add_wire(self.m1_stack, [pos1,pos2,pos3, pos4], widen_short_wires=False)
        self.add_path("m2", [pos3, pos4, pos5])
    ...
    def add_well_contact(self):
        ...
        #add nwell contact
        co_xoff = self.pmos1.rx()+self.m1_space+self.implant_enclose_body_active+self.extra_layer_enclose
        co_yoff = self.inv_inst.by()-self.well_enclose_active-contact.well.height
        contact_off=vector(co_xoff, co_yoff)
        self.add_contact(("active", "contact", "m1"), contact_off)
        height=self.minarea_active/contact.well.width
        self.add_rect(layer="active", 
                      offset=(co_xoff, co_yoff+contact.well.height-height),
                      width=contact.well.width,
                      height=height)
        
        extra_width = self.well_width-self.extra_layer_enclose
        extra_off= (self.pmos1.rx()+self.extra_layer_enclose,yoff)
        ...
        self.add_path("m1", [(co_xoff+0.5*self.m1_width,co_yoff), self.inv_inst.get_pin("vdd").lc()])
    ...
    def add_layout_pins(self):
        ...
        self.add_path("poly", [pos1, (self.starter_inst.lx()+1.5*self.m1_pitch, pos1.y)])
        self.add_contact_center(self.poly_stack,(self.starter_inst.lx()+1.5*self.m1_pitch, pos1.y))
        self.add_path("m1", [(self.starter_inst.lx(), pos1.y), 
                                 (self.starter_inst.lx()+1.5*self.m1_pitch, pos1.y)])
        ...
        self.add_path("m1", [(self.starter_inst.lx(), pos1.y), pos1])
        ...
        self.add_path("m1", [pos1, (self.pmos1.rx()+self.m1_pitch, pos1.y)])
        self.add_layout_pin(text="out",
                            layer="m1",
                            offset=(self.pmos1.rx()+self.m1_pitch-self.m1_width, 
        ...
        self.add_wire(self.m1_stack, [pos1, (self.starter_inst.rx()+self.m1_pitch, pos1.y),
                                      (self.starter_inst.rx()+self.m1_pitch, pos2.y), pos2], widen_short_wires=False)
        ...
        self.add_wire(self.m1_stack, [pos1, (self.starter_inst.rx()+2*self.m1_pitch, pos1.y),
                                      (self.starter_inst.rx()+2*self.m1_pitch, pos2.y), pos2], widen_short_wires=False)
