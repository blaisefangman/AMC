...
class tgate(design.design):
    ...
    def create_layout(self):
        ...
        self.width= self.nmos.height+self.pmos.height+4*self.m1_pitch+self.poly_space
    ...
    def connect_modules(self):
        ...
        #output routing
        self.out=vector(self.pfet1.rx()+2*self.m1_pitch, self.pfet1.get_pin("D").lc().y)
        ...
        self.add_path("m1", [pos1, mid_pos1, mid_pos2,self.out])

        #input 1 routing        
        self.in1=vector(self.nfet1.lx()-2*self.m1_pitch, self.nfet1.get_pin("S").lc().y)
        self.add_path("m1", [self.pfet1.get_pin("S").uc(), self.in1 ])

        #input 2 routing
        pos1 = self.pfet2.get_pin("D").lc()
        self.in2=vector(self.nfet2.lx()-2*self.m1_pitch, pos1.y)
        mid_pos2 = self.nfet2.get_pin("D").uc()
        mid_pos1 = (mid_pos2.x, pos1.y)
        mid_pos3 = (mid_pos2.x, pos1.y)
        self.add_path("m1", [pos1, mid_pos1, mid_pos2, mid_pos3, self.in2])
        
        #connect gates: nmos2 to pmos1
        pos1=vector(self.nfet2.lx()-self.m1_pitch, self.nfet2.get_pin("G").lc().y)
        self.add_path("poly", [pos1, self.pfet1.get_pin("G").lc()])
        
        pos2=vector(pos1.x, self.nfet2.get_pin("G").by()-contact.poly.height)
        self.add_contact(self.poly_stack, (pos2.x, pos2.y+self.co_via_shift))
        
        self.up_down_off= vector(pos1.x-contact.m1m2.width, pos2.y)
        self.add_via(self.m1_stack, self.up_down_off)
        height1 = max(contact.poly.height, contact.m1m2.height-self.v1_via_shift)
        width1 =  max(ceil(self.minarea_m1/height1), contact.poly.width+2*contact.m1m2.width)
        ...
        pos2= vector(self.pfet2.rx() + 2*self.m1_pitch-self.implant_enclose_poly, pos12.y)
        ...
        for off in offset:
            self.add_rect(layer= "m1", offset= off, width= width1, height= height1)
        ...
        for layer in layers:
            self.add_rect(layer= layer, 
                          offset= (self.nfet1.lx()-2*self.m1_pitch, 0), 
                          width= self.nmos.height+2*self.m1_pitch , 
        ...
        for layer in layers:
             self.add_rect(layer=layer, 
                           ...
                           width = self.pmos.height+2*self.m1_pitch+self.poly_space , 
