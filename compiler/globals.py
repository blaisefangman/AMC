...
def init_openram(config_file, is_unit_test=True):
    ...
    # Async does not utilize below features
    if OPTS.mode == "async":
        return
...
def read_config(config_file, is_unit_test=True):
    ...
    if (OPTS.output_name == "" and OPTS.mode == "sync"):
        ports = ""
    ...
    if (OPTS.output_name == "" and OPTS.mode == "async"):
        OPTS.output_name = "sram_{0}b_{1}_{2}".format(OPTS.word_size,
                                                      OPTS.words_per_row,
                                                      OPTS.tech_name)

    # If not using analytical delay, then we are running characterization
    try:
        # Check if characterize is defined, if not, set characterize
        OPTS.characterize
    except AttributeError:
        if OPTS.analytical_delay == False:
            OPTS.characterize = True
        else:
            OPTS.characterize = False
...
def end_openram():
    ...
    if OPTS.mode == "sync" and OPTS.check_lvsdrc:
        import verify
...
def setup_paths():
    ...
    try:
        OPENRAM_HOME = os.path.abspath(os.environ.get("AMC_HOME"))
    except:
        debug.error("$AMC_HOME is not properly defined.", 1)
    debug.check(os.path.isdir(OPENRAM_HOME),
                "$AMC_HOME does not exist: {0}".format(OPENRAM_HOME))
...
def set_default_corner():
    ...
    # Async does not use load or slew scales
    if OPTS.mode == "async":
        return
...
def import_tech():
    ...
    try:
        OPENRAM_TECH = os.path.abspath(os.environ.get("AMC_TECH"))
    except:
