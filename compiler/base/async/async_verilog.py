...
class verilog:
    ...
    def verilog_write(self,verilog_name):
        ...
        if self.mask:
            self.vf.write("  input [DATA_WIDTH-1:0] bm_in;\n")
        ...
        if self.power_gate:
            self.vf.write("  input sleep;             // active low power_gate\n")
        ...
        if self.power_gate:    
            self.vf.write("  always @ (negedge sleep) begin\n")
        ...
        if self.mask:
            ...
            self.vf.write("              $display($time,\" Memory keeps its old value at ADDR=%b\",addr);\n")
            self.vf.write("          end\n")
            self.vf.write("      end\n\n")
            self.vf.write("\n")    
